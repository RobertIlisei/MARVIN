 ...
 * `domain:romanian-tax-compliance`). Namespacing is what lets the
...
function domainTagsFromContext(workDir: string): string[] {
  ...
  if (/anaf|apia|efactura|e-factura|romanian|romania/.test(hay)) {
    tags.add("domain:romanian-tax-compliance");
  }
