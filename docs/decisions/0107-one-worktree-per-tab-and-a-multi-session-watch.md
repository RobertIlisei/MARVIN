...
## Addendum 8 (2026-09-11) — a review of what a week of real multi-session use left behind

Asked to look again because *"I still feel like there are broken things"*, with a project holding four open tab worktrees, fourteen unpushed merge commits on main, and thirty-eight session records. Two reviews plus direct measurement found six things that bite in ordinary use. All six are fixed here; the rest are recorded as known and unfixed at the end.

**1. A tab closed with *Merge* was dead forever.** `/api/chat` refused with `409 worktree-missing` whenever the session's checkout was absent — and absent is the *normal* end state of close-with-merge, which removes the tree and lets the sweep delete the branch. The recut that this ADR's reopen rule promises sat a dozen lines below, unreachable. The refusal now applies only to an **open** tab whose tree vanished underneath it; a closed one is recut from HEAD, which is what "reopening an integrated tab starts it fresh" was always supposed to mean.

**2. Pressing Stop was recorded as a failure.** `classifyTurnFailure` had two outcomes, failure and interrupted-by-restart, and a user cancel fell through to failure. The meta then read `outcome: "error"`, which put the tab in the Sessions pane's **Needs you** section under a red cross and replayed *"Claude Code process aborted by user"* as an error banner on every switch back into it. There is now a third outcome, `stopped`: not red, and deliberately **not** folded into `interrupted`, because that flag is what `GET /api/sessions/interrupted` offers for resume, and resuming a turn someone stopped by hand is the opposite of what they asked for. The transcript carries `cancelled: true` so the banner stays quiet, and auto-continue is skipped outright rather than relying on the error text not matching a transient pattern.

**3. Answering from the Sessions pane never cleared the row.** `PendingConfirmInfo` carries a turn id and a tool-use id but no session id, so the optimistic clear was applied with an empty key and landed on a phantom ledger entry. The row stayed in Needs you, the dock badge stayed lit, and with the live feed down nothing ever corrected it. The row passes its own id now.

**4. A slow snapshot resurrected answered confirms.** The snapshot reducer already protects a freshly registered turn with a grace window; pending confirms had none, so a response composed before an answer landed put the question back and re-lit the badge. Confirms answered locally are remembered for the same window and filtered out of any snapshot older than them.

**5. Reclaim and Merge from the Source Control panel ignored the busy guard.** `sweepWorktrees` and `mergeWorktree` both take `isSessionBusy`, and only two of seven call sites passed it. A closed tab running a **Sync** turn derives `empty` or `merged`, so Reclaim could delete the checkout and branch out from under a live turn — the precise case the guard exists for. The panel's routes pass it now.

**6. Boot recovery minted session records that could never be cleaned up.** `markInterruptedAtBoot` upserts a meta for any cut-off session that lacks one, with no title, shared tree, and no `closedAt` — an *open* record. Every restart produced one, and on the measured project eleven had accumulated, each holding a row in the seven-day watch window with a Resume affordance nobody wanted. A session with no meta was never a tab this sidecar was tracking, so the minted record is stamped closed; the chat route clears that the moment a real message arrives.

### Known, and not fixed here

A detached HEAD merges into nothing and reports *"Merged … into HEAD"*. An empty squash inside Prepare MR is misreported as a conflict and silently skips every branch after it. A worktree whose tab the client has forgotten can never be reclaimed by any path, because `session` state is exempt from every sweep. Prepare MR can leak a temporary checkout and an `mr/` branch if it throws before its cleanup is installed. **Sync** clears `closedAt`, so a finished row appears in two pane sections at once and leaves History permanently. A stale live-turn id can make a tab unclosable. Duplicate session titles get ordinals in the tab strip but not in the pane's rows. And nothing anywhere knows that a branch can be **protected on the remote**: Merge and Merge all fold into whatever is checked out, which on a project whose `main` allows pushes from *No one* produces commits that can never leave the machine. That last one is the next thing worth designing, not patching.
