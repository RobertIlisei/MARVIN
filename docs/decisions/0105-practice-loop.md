...
| 7 | Plan shape — `plan.horizontal` / `plan.vertical` (extractor v5, 2026-09-07): a plan stacked by layer versus one whose first milestone is a tracer bullet. The layers are the project's own areas, discovered from its graph (`discoverAreas`), never a vocabulary MARVIN ships; a `nudge`-tier rule fires on the `TodoWrite` that presents a horizontal plan | shipped 2026-09-07 | 201 of 399 sessions on a real project carried a layer-stacked plan before the rule; the verification window says whether the Phase 5 text and the nudge move that |
