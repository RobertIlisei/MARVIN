...
- **2026-09-21 — unreleased: MARVIN's own repo has one instruction file.** `AGENTS.md` was an untracked 12-day-old Codex port of `CLAUDE.md` (its 82 unique lines were stale text or mechanical Claude→Codex substitutions, some wrong — "Codex Agent SDK", `.Codex/skills/`); Codex ran 38 sessions here on it. Now `AGENTS.md` is the canonical, tool-neutral file generated from the current rules (graphify slash commands replaced by the `graphify` CLI), and `CLAUDE.md` is `@AGENTS.md` plus the Claude-only commands, hooks and settings. Every line of the old `CLAUDE.md` is present verbatim except the neutralised slash-command lines. The knowledge-graph builder now indexes `AGENTS.md` by default, for any project.
- **2026-09-21 — unreleased: CLAUDE.md, AGENTS.md, imports and nested instruction files ([ADR-0119](../decisions/0119-instruction-files-claude-md-agents-md-imports-nested.md)).** `resolveInstructionFiles` reads a directory's `CLAUDE.md` / `.claude/CLAUDE.md`, and `AGENTS.md` when there is no `CLAUDE.md` or when it carries distinct content (a near-copy — ≥ 50 % of its lines in `CLAUDE.md`; real copies 85–88 %, a distinct file 9 % — is skipped and reported). `@path` imports expand like Claude Code's, confined to the project by real path. A `PostToolUse` hook surfaces a subdirectory's files once per session when a Read/Edit/Write reaches it, reset on compaction, never under dependency/build/VCS/`.marvin` directories. New `marvin:instruction-files` skill for making duplicates one file, with the user's go-ahead. 20 tests; live on a real nested `apps/api/CLAUDE.md`.
- **2026-09-21 — unreleased: the base prompt from 29.4K to ~4.4K tokens, with MARVIN's own skills ([ADR-0118](../decisions/0118-one-prompt-per-session-and-settings-isolation.md) M2–M4).** Indexes load as a recent slice (Golden Rule 5 amended with the user's approval: docs whole, ADR titles / memory / backlog as their newest slice plus the tool that fetches the rest). `marvin-memory`/`-backlog`/`-obsidian` deferred behind ToolSearch. `personality.ts` 2,070 → ~210 lines: enumerated MUST triggers retired (current models over-fire on them; gates enforce what matters), procedures moved into six on-demand skills shipped with MARVIN as a `marvin` plugin (`core-skills.ts`), parsed formats kept verbatim. Checked against the old prompt on a clone of agri-saas-platform across an Ask, a Plan and a planted-bug fix: same outcomes, cheaper on two, and the new prompt actually consulted the advisor on a schema change the old one skipped; two soft triggers that slipped in the first draft (`systematic-debugging`, `graph_query`) were fixed by naming the moment, then re-verified. The app shows a quiet row when a turn starts over half its window. Agri fresh tab overall: 151,927 → **72,284** tokens.
- **2026-09-21 — unreleased: one system prompt per session, and settings isolation that is real ([ADR-0118](../decisions/0118-one-prompt-per-session-and-settings-isolation.md)).** A fresh agri-saas-platform tab sent 142,596 tokens on its first request and died on "Autocompact is thrashing". Measuring with the SDK's `getContextUsage` (now logged every turn as `runagent.context_baseline`; `sidecar/scripts/context-baseline.ts` for a fresh tab) found: the append differed between turn 1 and turn 2, and a resumed turn's append replaces turn 1's — so from turn 2 the model had no project context from MARVIN at all; and "isolation mode" was never on — omitting `settingSources` loads every source, so turns carried the user's Claude Code plugins (10 agents, hooks-bearing plugins), the project `CLAUDE.md` twice and Claude Code's auto memory. Now: the full append every turn, recorded by the SDK (`snapshot: true`, removing ADR-0073's transitional pin); mode, orientation, tree and plan in the `<system-reminder>` suffix, mode stated on every turn; `settingSources: ["user"]`, user plugins off in the flag layer, `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`. Agri: 151,927 → 139,761 tokens. Live: PLAN → AGENT switch on a resumed turn reaches the model, project codeword still visible. Refuted on the way: project allow rules do not bypass `canUseTool`. Not adopted: the SDK `skills` allowlist (cost 6.6K of tool schemas to save 1.8K).
- **2026-09-21 — unreleased: Agent SDK 0.3.278, graphify 0.9.65, and a blast-radius tool that had gone blind.**
  - *SDK 0.3.251 → 0.3.278* ([ADR-0073 addendum](../decisions/0073-agent-sdk-0-3-upgrade.md)). One default moved: `systemPrompt.snapshot` records turn 1's system prompt and replays it until compaction. Probe on this account: append `ALPHA` then `BRAVO` on resume → turn 2 answered `ALPHA`. MARVIN's append changes per turn (mode guidance; full project context on turn 1 only), so pin 3 is `snapshot: false` — transitional, until ADR-0118 makes the prompt stable per session. Not adopted, checked against the code: `omitClaudeMd`, `holdOnCacheImpact`, the new error codes, `thinkingTokens`.
  - *Trust by provenance* ([ADR-0117](../decisions/0117-mcp-trust-by-provenance.md)). `canUseTool` now reports `mcpServer.source`; a MARVIN-named tool from a non-`sdk` source confirms. Missing provenance keeps the name-based allow: subagents made 382 MARVIN-MCP calls through the gate and their calls were not shown to carry the field, so failing closed would have denied every scout's graph tools. `gate.mcp_provenance_missing` will say whether that fallback is ever used. Defence in depth — no reachable spoof exists today.
  - *`graph_affected` had been answering from 2026-08-13.* graphify moved its AST cache to `cache/ast/<version>/`, made paths relative, and stopped caching JS/TS entirely (`_JS_CACHE_BYPASS_SUFFIXES`); `call-index.ts` read only the flat top level. The index now comes from `graph.json`'s `calls` edges, oriented by call-site file (source = caller in 4,633 of 4,633 edges), with the versioned cache as the fallback. MARVIN: 4,633 edges in 47 ms; agri-saas-platform: 28,684 in 222 ms. 8 new tests.
  - *Graphs*: code 10,119 nodes / 21,655 edges / 575 communities, knowledge 1,988 / 2,681 / 173, both relabelled.
- **2026-09-13 — unreleased: the drain bound measures silence, not time since the result.** Two turns in one session ended `aborted by user` with nobody clicking Stop, each 15 min 2 s after a `result` deferred behind a live advisor, with the model mid-edit; the auto-continue rail refused at `attempt 9 > cap 3` and the tab went to STATE error. The ADR-0080 drain bound was an absolute timer. `BackgroundDrainBound` restarts on every subprocess event, clears when the ledger drains, and ends the turn on the captured result when it does fire. ADR-0080 amended; 3 tests.
- **2026-09-13 — v0.1.114: a layout loop the breaker cannot see gets a probe.** Ten minutes at 100 % on the main thread, force-quit. `sample` showed SwiftUI's lazy layout re-placing text children whose flexible-frame measurement never settles — no AppKit constraint pass, so the ADR-0062 breaker never fired, and no MARVIN frame to name the view. The session in front hydrates cleanly on relaunch, so the trigger was an interaction; source reading gave candidates, not a cause. `LayoutOscillationProbe` (`onGeometryChange`, no extra views) now labels the transcript stack, every transcript row and the Sessions list; 40 size changes in one second writes the label and its last sizes to the exceptions log. The next occurrence names its row ([ADR-0062](../decisions/0062-update-constraints-loop-identified-mitigated.md) Addendum 8). `swift build` clean.
