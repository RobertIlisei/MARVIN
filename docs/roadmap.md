...
- **Backlog overlap detection (ADR-0044 addendum)** — exact-slug dedup can't see two items describing the same work in different words, and un-gated capture (ADR-0047) makes those accumulate (the open rail went 50 → 200 for exactly this reason). `backlog_add` now flags live items resembling what was just parked, and `backlog_resolve` flags live items resembling what was just resolved. **Surface-only:** the pure `backlogSimilarity` / `relatedBacklogItems` return candidates for the user to judge; nothing mutates a sibling. Auto-reconciling was rejected — it would mutate items that already passed keep/dismiss with no new consent point, silently delete captured work whenever the judgement is wrong, and turn the parking lot into a self-driving queue (Golden Rule 1). Both write paths covered: the MCP tools phrase it as a question for the user, and the macOS panel shows a dismissible advisory after a manual add.
- **IDENTIFIED + MITIGATED (root cause still open): `_postWindowNeedsUpdateConstraints` crash ([ADR-0062](decisions/0062-update-constraints-loop-identified-mitigated.md))** — the instrumentation caught it on the first occurrence after install: `NSGenericException` — *"the window has been marked as needing another Update Constraints in Window pass, but it has already had more … passes than there are views in the window"*. AppKit's loop breaker for a non-converging layout pass, on the main window. The captured stack shows the loop closing **inside SwiftUI** — `NSHostingView._willUpdateConstraintsForSubtree` → `cancelAsyncRendering` → `setNeedsUpdate` → `setNeedsUpdateConstraints`, i.e. the constraint pass causes another constraint pass — and MARVIN creates none of the nested hosting views involved. Mitigated by registering `NSApplicationCrashOnExceptions=false` (registration domain, user-reversible): a stale frame beats losing the session, and every occurrence is still logged. **Root cause open** — what drives continuous invalidation of the main window. Ruled out *by experiment*: `RichText` re-setting its text storage (the equality guard holds) and the slash-popup geometry cycle (fixed, but the next crash was byte-identical). Historical detail below.
- **⚠️ (superseded, kept for the trail) `_postWindowNeedsUpdateConstraints` crash (2026-08-02, -08-05, -08-07)** — an uncaught NSException thrown from `-[NSWindow(NSDisplayCycle) _postWindowNeedsUpdateConstraints]`, reached from `NSHostingView.setNeedsUpdate()` during `NSHostingView.layout()`, ending at `+[NSApplication _crashOnException:]` (SIGTRAP). **Two fixes have failed.** The 2026-08-06 attempt froze `ChatInputView`'s `roomAbove` geometry measurement while the slash popup is open (a real geometry→state→geometry cycle, but evidently not this one); the 08-07 crash has a byte-identical stack and its `slice_uuid` matches the rebuilt binary, so the hypothesis is disproven, not merely unconfirmed. Root cause is still **unknown**: the `.ips` carries the backtrace but no exception `name`/`reason`, nothing reaches the unified log, and it fires after hours of uptime (42 h, then 3.7 h) rather than on a reproducible interaction. **Next step is instrumentation, not a third guess** — `CrashDiagnostics.swift` captures the exception's `name` + `reason` + symbols to `~/Library/Logs/MARVIN/exceptions.log` via two paths: a swizzle of `-[NSApplication reportException:]` on the live app class, plus `NSSetUncaughtExceptionHandler`. **The obvious approach — an `NSApplication` subclass named in `NSPrincipalClass` — is silently ignored under SwiftUI**, which installs its own `AppKitApplication` before the plist key is read; that was caught only because the hook stamps a session line saying whether it is armed. Keep that stamp: a diagnostic that fails quietly is worse than none. Open question for the user: whether to also set `NSApplicationCrashOnExceptions=NO` so the app logs and survives instead of dying. (b) *`ViewListTree.visitItem` SIGTRAP* in the file tree (report 2026-08-03, the fourth of this class): `outlineChildren` maps an empty directory to `nil`, so a folder could flip branch → leaf while keeping `id == path`, leaving `NSOutlineView` holding child row entries for a childless item. `FileNode.id` now encodes branch-ness, turning that shape change into a remove + insert. **Then the durable fix landed ([ADR-0061](decisions/0061-file-tree-flat-list-not-outlinegroup.md)):** `OutlineGroup` is retired entirely — `flattenFileTree` (MARVINLogic) produces a plain row array rendered by `List` + `ForEach`, with indent/chevron drawn by the row and expansion held as a path-keyed `Set<String>`. No `OutlineListCoordinator`, so all four crashes of that class are structurally impossible rather than individually patched. `FileNode` moved to MARVINLogic so the invariants are unit-pinned (25 new assertions) — previously every fix was verified by running the app and waiting to see if it died.
- **Check-back guard failed again — fixed ([ADR-0055](decisions/0055-checkback-promise-auto-arm-guard.md) addendum)** — a turn promised "I'll check readiness and run the Playwright verification in ~2.5 minutes", ended, and went silent. Two independent defects: (a) the detector never matched — the timed pattern capped the gap between `I'll` and `in` at 40 chars (the real clause is 51, so a promise failed to register *for being wordy*) and used `\d+`, which can't match `2.5`, though `parseDelaySeconds` always could; (b) `armedFollowThrough` was one boolean set by *any* `schedule_wakeup` **or** `run_background_job` — the turn had started the dev stack in the background, a server that never exits and therefore never fires ADR-0038's completion turn, so the guard was disarmed by a job that could not possibly discharge the promise. Coverage is now per-promise (`hasExplicitDelay` + `isCheckBackCovered`): a wakeup covers anything, a background job covers only open-ended promises. The lesson: the guard asked "was a mechanism armed?" when the question is "was THIS promise discharged?".
- **Backlog groomer — Phase 1 of the backlog loop ([ADR-0063](decisions/0063-backlog-groomer-review-not-execute.md))** — `backlog_groom` reviews the whole backlog and reports near-duplicates (reusing the ADR-0044 similarity scoring), provisional items never reviewed, items untouched for weeks, references to files that no longer exist, and HIGH-severity items left sitting. **Read-only by construction** — grooming reports, the user decides; the rendered report carries that instruction with the data, not just in the system prompt. **Three entry points, all read-only** (addendum 2026-08-12): the `backlog_groom` tool, a **Review button** in the macOS Backlog panel (findings render as annotations on the rows they concern, with a summary bar that says nothing was changed and counts findings hidden by the current filter), and **`/groom`** — a MARVIN-native slash command, since an in-process MCP tool appears in neither source the catalog is built from. Phase 2 (autonomously *working* backlog items) is deliberately unbuilt: it needs an amendment to ADR-0044 §5 plus a per-item `approved-for-auto` state, a definition-of-done per item as the external stopping condition, one-item-per-cycle, real verification, branch-only work and a hard stop — preconditions recorded in the ADR. Note the anti-Kanban rule that would need amending is §5, *not* Golden Rule 1: a single session looping on its own backlog is still a single assistant.
- **Find in file restored (⌘F)** — the native file viewer had no in-file search at all: the Find menu was removed with the WebView (ADR-0021 M5) and the replacement was left as a follow-up comment in `MARVINApp.swift`. `STTextView` already owned an `NSTextFinder` and implemented `performTextFinderAction(_:)`; nothing ever called it. Now wired via `FindCommands` — Edit ▸ Find with the standard ⌘F / ⌘G / ⇧⌘G / ⌘E, plus a magnifying-glass button in the viewer header (the menu alone is invisible, and that's twice this month a capability shipped without an on-screen affordance). The action is dispatched as a tagged `NSMenuItem` down the responder chain, because `performTextFinderAction` reads its action from `sender.tag` and asserts on anything else; the header button additionally focuses the editor first so "Find in File" can't end up searching the chat composer.
- **Backlog panel reachable when empty (⌘⇧B)** — the tray chip is gated on `backlogOpenCount > 0`, so a project with an empty backlog had *no route* to the panel, and therefore none to the groomer's Review button either. Added **View ▸ Backlog (⌘⇧B)**, posting `.marvinRequestBacklogPanel`. Uses `.onReceive` rather than the `addObserver` pattern the sibling notifications use: those all mutate `model.*` (an @Observable reference, safe to capture), whereas `backlogPanelOpen` is `@State` on the view struct and assigning to it from a captured copy is a classic silent no-op.
- **Backlog `kind` + `blocked` ([ADR-0064](decisions/0064-backlog-kind-and-blocked.md))** — measured a real backlog (430 items, 56 live) before choosing a taxonomy, and the data changed the answer twice. Type can't be inferred from titles (leading tokens are nouns/identifiers, not verbs), so classification is explicit. And reading all 56 surfaced two categories a generic bug/improvement/chore split would have hidden: **`investigate`** (~1 in 5 — the output is a decision, not a diff) and **`docs`**. One item turned out not to be a kind at all: ~5 items wait on something outside the repo, so **`blocked`/`blockedOn` is its own axis**, not a status value — a blocked bug and a blocked feature are both blocked. Both must change behaviour or they're decoration: the groomer gains `aging-bug` (14-day bar — a bug doesn't become less true by being ignored) and `blocked-without-reason`, plus two **exemptions** that matter more than the rules — `investigate` and `blocked` items are never called stale, because nagging about things the user can't act on trains them to ignore the whole report. **No backfill**: 430 existing items stay `unspecified`, parsing is back-compatible, and an omitted kind on re-add preserves what the user set.
- **CI safety — both halves done** — (a) a tag push matched neither of `test.yml`'s branch filters, so cutting a release ran **zero** tests (four shipped red); `test.yml` is now `workflow_call`-able and `release.yml`'s build job `needs: test`, so the gate can't drift from what development verifies. (b) CI is now audit evidence ([ci-status.ts](../sidecar/packages/runtime/src/ci-status.ts)): the session auditor's packet gains §G and a `shipped-red` finding category. The load-bearing case is **`stale`** — a green run for a *different* commit is reported as saying nothing, never as a pass, so an older green can't vouch for a commit it never built. `unknown`/`stale` explicitly cannot ground a finding either: absence of evidence isn't evidence of failure.
