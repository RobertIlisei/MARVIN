...
- **Self-scheduled wakeups — make "I'll check back" real** ([ADR-0031](./decisions/0031-self-scheduled-wakeups.md), development branch). MARVIN was narrating async follow-through ("Monitor armed — I'll continue when it reports") it couldn't perform: a turn is only ever started by `POST /api/chat`, so nothing re-invoked it. New `schedule_wakeup` / `cancel_wakeup` / `list_wakeups` tools (`marvin-control` MCP server) + a bounded server-side scheduler (`wakeup-scheduler.ts`) that persists to `<dataDir>/wakeups/<projectId>.json` and re-arms on boot via `instrumentation.ts`. A fired wakeup starts a real turn through the shared `runDetachedTurn` orchestrator (extracted from the chat route), resuming the session. Rails: 60 s–24 h delay, ≤5 pending/session, chain-depth ≤8, past-due fires once, >24 h dropped. `personality.ts` flipped from "you are not a scheduler" to the real tool. Delivery reuses existing `turn-registry` + resume plumbing (visible on next view); live-push to an idle tab is a future extension.
