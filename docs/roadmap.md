...
- **A project's instruction files, read the way other agents write them (2026-09-21, [ADR-0119](./decisions/0119-instruction-files-claude-md-agents-md-imports-nested.md)).** MARVIN injected only the root `CLAUDE.md`, verbatim: an `AGENTS.md`-only project (Codex, Cursor) gave it nothing, an `@AGENTS.md` import arrived as a literal line, and since ADR-0118 nested files reached nobody. Now `CLAUDE.md` and/or `AGENTS.md` load with imports expanded inside the project; a near-copy `AGENTS.md` (≥ 50 % of its lines in `CLAUDE.md` — real copies measure 85–88 %) is skipped and reported with a pointer to the new `marvin:instruction-files` skill; a subdirectory's files surface once per session when work reaches it. Live: a turn quoted a rule from `apps/api/CLAUDE.md` it never opened.

- **Context budget: a fresh tab starts at 71 % of its window (2026-09-21, [ADR-0118](./decisions/0118-one-prompt-per-session-and-settings-isolation.md) — Milestone 1 landed).** *"a new session with Shared workspace should be a fresh one completely."* It was fresh, and it still thrashed: session `bf45b11e` on agri-saas-platform sent **142,596 tokens** on its first request, auto-compacted six times in eleven minutes (each compact left ~20K of messages, the next request was back at 156–167K), and died on the SDK's "Autocompact is thrashing". Compaction frees messages only; the fixed load is ~33K Claude Code preset + ~29.5K MARVIN base prompt (91 MUST, 43 MUST NOT, 54 ADR citations) + ~28K project context + ~10K skills + MCP tools. Plan: measure with a `getContextUsage` probe → one stable system prompt per session with per-turn deltas in the `<system-reminder>` suffix (also fixes: from turn 2 the model has had no project context at all) → project context by index → base prompt ≤ 8K → defer non-graph MCP servers, list only enabled skills, a baseline-over-50 % warning. Target ≤ 75K on agri's first request. **Landed:** one recorded system prompt per session with mode in the turn suffix (turn 2 had been losing the project context entirely); real settings isolation — MARVIN had been loading the user's Claude Code plugins, the project `CLAUDE.md` a second time and Claude Code's auto memory; per-turn `runagent.context_baseline` + `sidecar/scripts/context-baseline.ts`. Agri: 151,927 → 139,761; indexes as a recent slice (Golden Rule 5 amended) → 111,245; three MARVIN MCP servers deferred → 107,901; base prompt 29.4K → ~4.4K tokens with six `marvin:*` skills → **72,284**. A turn over half its window now shows a row saying so. Also: SDK 0.3.278 gives Fable 5.1 a 1M window. All four milestones done.

- **Agent SDK 0.3.278 + graphify 0.9.65 (2026-09-21, [ADR-0073 addendum](./decisions/0073-agent-sdk-0-3-upgrade.md), [ADR-0117](./decisions/0117-mcp-trust-by-provenance.md)).** SDK: a new default (`systemPrompt.snapshot`) would have frozen turn 1's prompt — pinned off, transitionally, measured live; MARVIN MCP tools from a non-`sdk` source now confirm. graphify: both graphs rebuilt and relabelled; `graph_affected` reads `graph.json`'s directed `calls` edges, because 0.9.65 caches no JS/TS and the tool had been answering from entries frozen on 2026-08-13.

- **A turn ended in error fifteen minutes after its result was deferred — twice (2026-09-13, [ADR-0080](./decisions/0080-background-subagents-and-builtin-readonly-agents.md) amended).** *"check what is causing marvin to have errors because it stops working."* Both `turn.error: aborted by user` records sat 15 min 2 s after a `runagent.result.deferred` (an advisor still live) with the model mid-edit, and the auto-continue rail refused at `attempt 9 > cap 3`. The drain bound was an absolute timer: not restarted by activity, not cleared when the advisor settled, and its abort took the error path. `BackgroundDrainBound` now measures silence — any event restarts it, a drained ledger clears it — and a fire ends the turn on the captured result with `runagent.bgdrain` telemetry. Three fake-timer tests.

- **A layout loop the breaker cannot see, and the probe for it (2026-09-12, [ADR-0062](./decisions/0062-update-constraints-loop-identified-mitigated.md) Addendum 8).** Ten minutes at 100 % on the main thread, force-quit. `sample`: SwiftUI lazy layout re-placing text children whose flex-frame measurement never settles — no constraint pass, so the ADR-0062 breaker was blind, and no MARVIN frame to name the view. The session in front hydrates cleanly on relaunch, so the trigger was an interaction. **Cause not yet identified.** `LayoutOscillationProbe` (`onGeometryChange`, no extra views) now labels the transcript stack, every transcript row, and the Sessions list; 40 size changes in a second writes the label and its last sizes to the exceptions log. The next occurrence names its row; the fix follows that measurement.

- **A plan that says where its steps are (2026-09-12, [ADR-0068 addendum 5](./decisions/0068-plan-dedupe-provenance-and-negative-claims.md)).** User, two screenshots of `0/11` under a reply naming two commits: *"now it's even worse with the plan."* It was not the day-old spine work, which was not running — the installed bundle predated it by three hours and no stored spine carried the new watermark. Running the parser's own patterns over the file reproduced the eleven rows exactly: three `## Scope` bullets that *index* the steps, eight numbered `## Definition of Done` criteria, and the nine real vertical slices under `## Steps` parsed not at all. Three causes, each independent. A **blank line** ended the criteria block, so the only line ever excluded was the heading — addendum 4's rule had been working by accident on plans that omit the markdown gap. The criteria scanner accepted only bullets while the step matcher beside it also accepted `1.`, so a numbered Definition of Done ended its own block on its first line; nine of 397 plan files carry one. And the steps were written `**1. Tracer slice — …**`, which the matcher never saw because emphasis is stripped only after the match. Fixed with one shared `isListItemLine`, a blank-line rule that reads position (before the first item it is the gap, after one it is the separator), leading emphasis allowed in both matchers **and** in `render`'s `markerRE` so checkboxes still land, and the section rule: when a plan names `## Steps` or `## Implementation`, that is where its steps are. `Milestone` is excluded from that vocabulary — sibling `### Milestone A/B` headings cut a real 16-step plan to four. Measured across all 397 plans: 372 identical, 25 changed, every change a reduction, none below two steps. 928 Swift assertions, 16 new. **Not yet done:** an already-corrupted spine does not self-heal, because `redriveSteps` bails when the counts match; repairing one is deliberate and evidence-led.

- **A tab branch is never published by MARVIN (2026-09-12, [ADR-0109](./decisions/0109-batch-integration-and-metered-ci.md) second amendment).** Two tabs each pushed their `marvin/tab/…` branch and opened an MR (!215, !216) — the metered-CI confirm asked, was allowed twice, and the project paid two pipelines for one piece of work. In a worktree tab the answer is always the same, so it is now a **deny** ahead of the confirm: any `git push` and any `glab mr` / `gh pr` create or merge, naming Merge all / Prepare MR. A shared tab is denied only when a `marvin/` ref is what gets pushed; the user's own branch keeps the normal ladder. The prompt says so too. Four gate tests.

- **The plan spine has two writers now (2026-09-11, [ADR-0116](./decisions/0116-the-spine-has-two-writers.md)).** User, with two screenshots: *"why does marvin say that he finished implementation but the plan still has opened items unfinished … we tried to fix this issue."* Reconstructed from the session rather than read off the code: fourteen `TodoWrite` calls, and the stored spine is byte-for-byte the ninth. The boundary is exact — the last batch of the last turn the user typed into landed, and the five that followed, across four `[scheduled wakeup]` turns, were lost. Three mechanisms, in order. `writePlanState` had **no caller in the sidecar**, so a client-derived record was tracking a server-driven process and a turn nobody was watching could not advance the plan; meanwhile the neighbouring tab's spine updated normally, which is why two sessions disagreed about whether MARVIN was working. Hydration then re-imposed the stale spine over the fresher replay on every switch back, ADR-0052's *"so it wins"* being right about structure and wrong about progress. And the ADR-0057 guard read this turn's `TodoWrite` as its evidence — the closing batch said 14 of 14 completed, so the check built to catch a premature "done" was reading the premature "done". Underneath: an auto-compaction four minutes before the close, 968k tokens to 22k, after which the batch came back **untagged and reworded from the summary**, and the `[N]` ordinal is the join key. Fixes: the sidecar projects each batch into the spine by the deterministic tag join only (no fuzzy half — that stays in `PlanModel.swift`, ADR-0112), refusing what it cannot join rather than guessing; the guard unions spine and batch, and names an untagged close for what it is; a `lastTodoAt` watermark orders the two sources instead of ranking them, leaving legacy spines untouched; the prompt requires re-reading the plan before the first `TodoWrite` after a compaction. 18 + 6 sidecar tests, 9 Swift assertions, suite 1412 green. **Not yet done:** the live check on the rebuilt app, and nothing yet reconciles a spine the server refused to join with what the client would have matched fuzzily.

- **One vocabulary for the left pane (2026-09-11, [ADR-0114](./decisions/0114-one-vocabulary-for-the-left-pane.md)).** User: *"use /apple-design … do not remove any functionalities instead add more."* Measured first, across all seven panes: **five** button styles, **five** section headers, **four** corner radii, **three** registers of empty state, and a `PaneNotice` type that exactly one pane used. Familiarity failing at the level of the whole sidebar. A shared vocabulary now lives in `MARVINLogic` (the decisions, asserted) and `MARVIN/Pane*.swift` (the drawing, ruleless), admitted by one rule — three or more panes need it, and the signature carries no pane-specific parameter — which is why row *content* stayed per-pane and row *shape* is a modifier. Then all seven adopted it, one per commit. What that turned up: Search opened a result's file but **not its line**, though the line sat two lines from the call; **Replace All** had no confirm, no preview and no undo while writing file by file; Skills and Plugins both shipped an enable control that was a plain glyph, one of which loads MCP servers; a refused plugin toggle did nothing at all; Practice's header truncated its own labels and styled *Reset findings…* as a hyperlink; and dismissing a Practice finding was a one-way door, so **undo** was built. **Declined, with the reason recorded:** uninstalling a plugin, which would mutate a registry Claude Code owns. 894 Swift assertions (39 new), sidecar 1382. **Not yet done:** the narrow-width and keyboard passes, which need the rebuilt app in front of a person.

- **A truncated label is a broken control (2026-09-11, [ADR-0112](./decisions/0112-one-place-for-each-thing.md) amended again).** The branch chip's popover rendered `Switch to ow…` and `Open in Ses…`: four controls of four different kinds sharing one row in a popover with no width, refereed by a layout pass. Rebuilt from what each control is — a fixed 340pt to match the posture popover beside it, one full-width primary action that cannot truncate, *Edit* moved onto the Lane row it edits, the leaving-the-branch choices stacked so *Keep for integration & switch* stays a whole sentence, and navigation demoted to links in a footer.

- **Six things a week of real multi-session use broke (2026-09-11, [ADR-0107](./decisions/0107-one-worktree-per-tab-and-a-multi-session-watch.md) Addendum 8).** User: *"I still feel like there are broken things."* Measured on a project with four open tab worktrees, fourteen unpushed merge commits and thirty-eight session records. A tab closed with **Merge** was dead forever, because the chat route refused a missing checkout before reaching the recut that close-with-merge always makes necessary. Pressing **Stop** was recorded as a failure, so the tab sat in *Needs you* under a red cross and replayed "aborted by user" as an error banner on every switch back; there is a third outcome now, `stopped`, deliberately not resumable. Answering a confirm from the pane wrote its optimistic clear under an empty session key and never cleared the row. A slow snapshot put answered confirms back and re-lit the dock badge. **Reclaim** and **Merge** in the Source Control panel skipped the busy guard, so a closed tab running a Sync turn could have its branch deleted mid-turn. And boot recovery minted an *open* session record for every restart, eleven of which had piled up unreachable. Sidecar green, 855 Swift assertions. **Next, not patched:** nothing in the merge path knows a branch can be protected on the remote.

- **A control has to look like a control (2026-09-11, [ADR-0112](./decisions/0112-one-place-for-each-thing.md) amended).** User: *"nobody knows the Prepare MR, Merge All, Reclaim are buttons, they look like normal text"*, and *"clicking anything in Sessions, just selects the whole pane"*. Every action in both panes was a plain-styled button with a tinted label, in panes full of tinted *status* text — so no word could be told from another. One chip vocabulary now covers both panes, with three roles, a press cue on pointer-down, hover, and reduced-motion respect; the Worktrees header gets a hierarchy (neutral *Merge all*, accent *Prepare MR…*, *Reclaim* in an overflow menu). The Sessions pane's whole-pane focus ring was ADR-0112's own `.focusable()` on the scroll view: the ring moves to the focused row, and *Ready to integrate* rows became clickable and now lead with their branch, since six rows shared one session title. Outcomes carry a kind and split the sentence from the machine detail, and a conflicted merge names the files instead of the command MARVIN ran. Found while testing: `git status --porcelain` was read through a trimming helper, shifting the first line's path one character, so `.marvin/memory.md` read as `marvin/memory.md` and counted as a merge blocker.

- **An integration is a job you can watch, and its commit counts what is in it (2026-09-11, [ADR-0109](./decisions/0109-batch-integration-and-metered-ci.md) amended twice).** User, on a live six-branch Prepare MR: *"i got nothing, no status update, no progressing tracking, nothing, how do i know it's working or not?"* — the sheet closed, two buttons dimmed, and the sidecar was three minutes into a squash, a commit under a compile-and-test hook, and a push, with no surface saying so. A run is a **job** now: registered, moved through `preparing → squashing → committing → pushing`, finished with its summary, readable for ten minutes after. A git-free `GET /api/worktrees/job` is polled every 1.2 s and the Worktrees section shows *"Squashing 3 of 6 · marvin/tab/fix-flags · 1m 12s"* / *"Committing — running the project's commit hooks · 2m 03s"*; because the record is the sidecar's, a refresh finds a run this app never started and the outcome survives a timeout, a project switch or a relaunch. The same run also exposed a lie: it stopped at a conflict on the second branch, then pushed a commit titled *"integrate 6 branches"* carrying one, and opened a merge request for it. A generated message is now built from what applied, and a run stopped by a conflict does not push at all — the branch is on disk and the summary names the branch to Sync. Sidecar 1375 green, 837 Swift assertions.

- **Merge refuses only what it would overwrite; Prepare MR waits as long as the sidecar does (2026-09-11, [ADR-0109](./decisions/0109-batch-integration-and-metered-ci.md) amended).** User: *"fix it so the check only refuses on overlapping files"* — a nine-file branch could not close with Merge because two ADRs from another tab were unsaved on main, files the branch never touched. `mergeBlockingPaths` now intersects the dirty tracked paths with the branch's diff and names the files; Merge all checks per fold. Same afternoon, *Prepare MR failed: the request timed out* — the squash commit was running the project's compile-and-test hook past the client's 30 s while the sidecar carried on and the result was lost. The call now gets 13 min (the sidecar's own commit + push budgets), a timeout reads as "still running", the outcome is logged, a typed `mr/<date>-` prefix no longer doubles, and the temporary tree is prepared like a tab's so the hooks can run. 41 tests in the three suites, `swift build` clean.

- **The backlog is the shared task list (2026-09-11, [ADR-0113](./decisions/0113-the-backlog-is-the-shared-task-list.md)).** Two isolated tabs fixed the same defect on the same afternoon, byte-for-byte, one of them shipping a regression test the other never saw. From the transcripts: the item was created in the root store at 17:41:47 as *open*; the second tab grepped `.marvin/backlog/` **inside its own worktree** at 17:42:52 — a snapshot cut with the branch, where the file did not exist — and started fixing; nothing ever moved the item to *doing*, because no tool could; it went open → done at 17:54 with no word to the tab mid-fix on the same file. Three fixes: items are **claimed** (`claimedBy` / `claimedBranch`, `backlog_claim`, holder shown in listings and in the near-duplicate refusal — *"already being worked in tab fix-the-thing"*, the panel's *Plan* claims for its tab and offers Switch on a held item); the worktree's backlog copy is **never read** (the gate turns back `Read`/`Grep`/`Glob`/`Bash` aimed at it, in every mode, naming `backlog_list`); a **resolution notice** goes to every other live tab whose branch touches a path the item names, injected mid-turn or queued. Sidecar 52 backlog tests green, 826 Swift assertions. **Not yet done:** the live check with two rebuilt tabs racing on one item.

- **One place for each thing: the multi-session chrome (2026-09-10, [ADR-0112](./decisions/0112-one-place-for-each-thing.md)).** User, with four screenshots: *"i get lost in all these, it's not clean or fluid or easy to use and interact with, UI/UX is a mess."* Counted: four ways to switch a session, two New buttons, three places to choose own-branch vs shared under three names, a session hash in the header, four stacked bars at the bottom, five posture chips in three styles, and a settings form living in a popover. Six milestones, client-only but one watch-row field: the tab strip is the only switcher and the `+` beside it the only New; the clock menu is History (closed sessions only); the header is project then branch chip; one vocabulary, "own branch" and "shared"; a glyph per tab state; the chip popover is facts plus two actions with the lane editor in a sheet; posture is one pill; an error with nothing to retry is a toast; a tool confirm is a card in the tray, never a sheet, and the same card answers inline from the Sessions pane's Needs-you row; the pane has five sections and arrow-key navigation. Swift green; sidecar green. First live check: four tabs from one prompt were indistinguishable and the way between them was gone — History now carries Previous / Next and duplicate titles get an ordinal; and the app crashed on ADR-0062's loop through AppKit's *layout-pass* counter, which the breaker had never hooked ([ADR-0062 addendum 7](./decisions/0062-update-constraints-loop-identified-mitigated.md): `setNeedsLayout:` is deferred on the same budget, and the selectable notices in left-pane scroll views that oscillated lost `.textSelection`). Second live check: the strip's fixed-width tabs cut the fourth off with nothing to step or list — tabs now compress to fit, ‹ › steppers and an overflow list sit at the end of the strip, and duplicate titles lead with their ordinal. **Not yet done:** a gate denial still renders in the transcript's error red (it is the tool's error result).

- **Nothing survives a tab unless it holds a commit (2026-09-10, [ADR-0111](./decisions/0111-nothing-survives-a-tab-without-a-commit.md)).** User: *"we can easily create multiple stray branches if we just click a new session and type something, stop it, close it and the branch just remains there. Also managing work when we finish multi session is not clean … If we are to do this we need to do it properly."* Measured first: 17 session worktrees on the project, 16 for closed tabs, 12 merged, 3 empty, 0 ever swept — the sweep ran from one button, and the close path's "reclaim silently" sent `keep`. Five milestones: the close decision remade from the sidecar's derived state (empty and merged trees discarded without asking, keep commits work first, Merge names its target) with the sweep at boot and after every close; a `git merge-tree` preview so the first conflict is known before the first merge; **Sync**, one turn in the owning tab's worktree that merges the target in and resolves; a Ready-to-integrate section in the Sessions pane with Merge / Merge all / Prepare MR and conflict chips in the sheet; branches named by their first commit; an integrated tab reopening on a fresh tree. Front-end review alongside: attachment-path tab titles fixed, sheet sort fixed, two items noted for a later pass. Sidecar suites green, 826 Swift assertions. **Not yet done:** the live check on the rebuilt app.

- **A plain chat is one click again (2026-09-10, [ADR-0107](./decisions/0107-one-worktree-per-tab-and-a-multi-session-watch.md) Addendum 7).** User: *"a way when opening a new session, to be separate of our multi-session-branches … like a normal chat that marvin had previously."* The shared-checkout tab was that chat, reachable only through a Settings default and a header chip. The `+` and **New** controls are now split buttons — click for the default, menu for **New Chat — shared checkout** / **New Isolated Tab — own branch** — and File gains the same two commands with ⌥⌘N / ⌃⌘N.

- **Prepare MR — the day's tab branches, one squash commit, one push, one merge request (2026-09-10, [ADR-0109](./decisions/0109-batch-integration-and-metered-ci.md) amended).** User: *"find a way to understand what were the latest branches we worked on … group them into a commit and push to gitlab for a MR"* — then, watching MARVIN do it by hand and stall at the `glab mr create` confirm while they were away: *"if i'm away, no work gets done."* `prepareMergeRequest` squashes the chosen `ready` branches, oldest first, into one commit on `mr/<date>-<name>` inside a disposable worktree (the user's checkout is dirty with `.marvin/*` always and is never entered), stops at the first conflict keeping what applied, and — only on the sheet's toggles — pushes and opens the request from the push with the target detected from the upstream. Tab branches untouched; the metered-CI decision moves to a toggle the user sets in advance. The Worktrees section now sorts by tip date and shows `+N −M`. Five tests. Still open: the reconcile is synchronous git on the event loop.

- **A graph query from a worktree tab confirmed on auto — again (2026-09-10, [ADR-0107](./decisions/0107-one-worktree-per-tab-and-a-multi-session-watch.md) Addendum 6).** Addendum 5 had stopped the `cd <root>` and read confirms; the survivor was graphify's incremental-detect snippet, which *writes* `graphify-out/.graphify_detect.json` in the main checkout, so the containment rule fired correctly on the wrong target. `graphify-out/` is MARVIN's per-project graph, git-ignored and read every turn (Golden Rule 7), not the user's checkout; it is now carved out of `mainTreeRedirect`'s main-tree test the way `.marvin/worktrees/` already was. Reads and writes there fall through in every permission mode; a command that also touches anything else in the main tree still confirms. Three gate tests, 1343 green.

- **Branch picker denied every switch; the Source Control panel said "(not a git repository)" on a healthy repo (2026-09-10, [ADR-0012](./decisions/0012-source-control-mutation-channel.md) amended).** Two bugs from one screenshot pair, both settled by measuring. *Switch:* the dirty-tree `deny` was tripping on MARVIN's own `.marvin/*` bookkeeping — seven modified files, nothing else — so no branch was ever reachable from the picker, and the footer printed the raw JSON envelope. Now a **warn confirm** ("uncommitted changes will carry over"); git's own overwrite refusal is mapped to `409 local-changes-would-be-overwritten` naming stash; the picker footer shows the route's sentence. *Panel:* `/api/worktrees` reconciles every tree with `execFileSync` (~1.5 s for 15 trees) and the panel fires it in parallel with status, so the status probe's 2 s timer fired late and its failure was reported as `not-a-git-repo` — which the panel then kept, since an `enabled: false` answer counted as settled. Now the route answers `probe-failed` for a timeout or spawn error and only `not-a-git-repo` for git's own exit 128; `runGit` re-arms once instead of calling a late timer a hang; the panel retries a probe failure after 2 s and never treats `enabled: false` as final. *Also:* a session worktree records **`baseRef`** — the branch HEAD was on at the cut — and the tab popover shows "Cut from `main @ 244d5a8`" instead of a bare sha; the draft note names the current branch. Left for later: the reconcile is still synchronous git on the event loop.

- **Close-with-merge could not commit: `spawnSync git ETIMEDOUT` (2026-09-10).** Six tabs on a real project, all finished, none closable with *Merge into my branch*. Measured in the worktree rather than read from the code: `git status` showed `A  node_modules` and `A  apps/web/node_modules` — MARVIN's own symlinks, staged. Two causes, both ours. The project ignores `node_modules/` and the trailing slash means *directories only*, which a symlink is not, so every tab worktree was dirty from birth and a merge-on-close always took the commit-first path; `git add -A` then staged the links and `git commit` ran the project's `pre-commit` — a Java compile plus its fast test band — against a **30 s** `execFileSync` in the route handler, which timed out and held Node's event loop for the whole wait. Now: `prepareSessionWorktree` writes each linked path, anchored, to the clone's `info/exclude` (never the project's `.gitignore`), so a symlinked dir never appears in status again; the close route unstages the links a previous attempt caught; and the work-in-progress commit is **`commitWorktreeWorkInProgress`** — asynchronous, ten minutes, hooks honoured, and a hook's own words in the failure message. Existing worktrees repaired in place. 38 worktree tests green.

- **A worktree isolates the filesystem, not the machine (2026-09-10, [ADR-0110](./decisions/0110-worktrees-isolate-the-filesystem-not-the-machine.md)).** User, after a cross-session test failure: *"check if our marvin code mistake and fix it, or if it's my mistake and help me prevent it next time."* Neither, exactly. Three tabs, three worktrees, three branches — and **one Postgres container between them**: `~/.testcontainers.properties` (a HOME-level file, nothing to do with the project) had `testcontainers.reuse.enable=true`, and reusable containers are keyed by config hash and shared across every JVM on the same Docker daemon. Two of the suites `DROP TABLE` in schema setup, so they were also corrupting each other's data whenever they overlapped; the visible failure was a connection refused against a valid port mapping with nothing behind it. ADR-0107's isolation did its job — the shared resource was outside all of it. The gap was that nothing said so, and there was no lever even for a user who knew. Now: the worktrees guide names the class (Docker, host ports, databases, global caches, `$HOME`) with this incident as the worked example and the two symptoms that give it away; and `.marvin/worktree.json` gains an **`env` map** applied to every turn in a worktree-mode tab. A Testcontainers rule would have been one line and is exactly what Golden Rule 6 forbids — the next collision is a port or a database name — so the project declares what makes its runs independent and `env` is never auto-detected, unlike `symlinkDirectories`. 1328 sidecar tests green.

- **Line numbers touching the file text (2026-09-10).** Measured off the user's screenshot rather than guessed: the rightmost pixel of a line number and the leftmost pixel of the text were **one pixel apart**. Two causes, both ours. The 3pt diff strip is a subview of the ruler, so it sits INSIDE upstream's 6pt trailing inset and eats half of it; the inset is now sized to clear it. And `STLineNumberRulerView` grows `ruleThickness` from the widest number in the **currently visible** range and only ever grows it, so a long file is sized for two digits until a scroll reaches line 1,247 — MARVIN now sizes the gutter up front from the file's real line count in the ruler's own font, and re-sizes it when a new file loads.

- **Integration is batched, and spending CI minutes asks first (2026-09-10, [ADR-0109](./decisions/0109-batch-integration-and-metered-ci.md)).** User: *"working 5 sessions and all commiting and pushing and merging to main, each with his own MR, means 5x~45 min + compute just for a MR to main branch."* The arithmetic is right and ADR-0103 had already measured the terms on this project: a request costs ~10 pipeline-minutes to open and **~48 compute-minutes** to merge to the default branch, against a 400/month allowance already exceeded and paid in cash — so five branches one-request-each is ~240 minutes against ~48 batched. ADR-0103 built the cheap path (`mergeWorktree` folds a branch in locally and never pushes) but two things had gone stale: it merges **one** branch at a time, written when branches came from implementers a few at a time rather than from every chat tab (ADR-0107); and **nothing stopped the expensive path** — the policy hard-denies `npm publish` and tag pushes on irreversibility grounds, while `glab mr create` appeared nowhere, so five tabs in `auto` mode could open five requests unattended. Now: **`mergeAllWorktrees`** folds every `ready` branch in one pass, oldest first, reporting what it skipped and why, stopping at the first conflict with the tree clean and the branch intact (an open tab's tree is never touched — that decision belongs to the close dialog); surfaced as **Merge all** in Source Control ▸ Worktrees. And **`classifyMeteredCiRisk`** gates the three acts that cost money — opening a request, merging one, running a pipeline by hand — as the fourth member of the confirm family that survives `auto` mode, denying with no UI attached. `git push` is deliberately not gated: it starts no pipeline here and is how work is kept safe, and a confirm there would train the user to click through the one that matters. 1322 sidecar tests green.

- **The constraint-pass breaker was armed and had never fired (2026-09-09, [ADR-0062 addendum 6](./decisions/0062-update-constraints-loop-identified-mitigated.md)).** Fifth `NSGenericException` of the same family. User: *"aaand marvin just crashed"* — `ps` said otherwise first: **100.1 % CPU, `STAT R`**, the spin signature, `sample` showing the main thread wholly inside SwiftUI's layout engine; it threw a minute later. The decisive measurement was a **count**: 8 launches logging `breaker: ARMED`, 5 fatal exceptions, **0** `breaker tripped` reports. Addendum 5's mitigation had never engaged. Its `passDepth > 0` guard is fed only by the hook on `NSWindow.updateConstraintsIfNeeded`, and that method is nowhere on the raising stack — every crash arrives through the display cycle's LAYOUT phase (`NSDisplayCycleObserverInvoke → NSWindow.layoutIfNeeded → _layoutViewTree → layoutSubtreeIfNeeded → NSHostingView.layout → LazyLayoutViewCache.invalidateSize → setNeedsUpdateConstraints → throw`). AppKit runs constraints from two entries and counts passes from both; the breaker recognised one. `layoutIfNeeded` now opens a pass too, looked up by selector name (it lives in a category) and verified to resolve before shipping. The session-start line names both entries. **Proof will be** a `breaker tripped` entry with no `.ips` after it.

- **A nav bar through the open sessions (2026-09-09).** User: *"we do not have a nav bar through the sessions, the opened ones."* With seven tabs open, six of them opened from the same prompt, the strip truncated every title to "Plan the implementation …", a tab past the right edge could not be reached at all, and nothing bound a key. The editor's file tabs solved this on 2026-08-31 with ‹ › arrows; the chat strip never got it, and copying it wholesale was the wrong move — arrows need content-vs-viewport widths, which means `background(GeometryReader)`, which means extra subviews in a window whose view COUNT is what AppKit's runaway-pass breaker budgets against (v0.1.93's `WidthReporter` fixed a storm and shipped a crash doing exactly that). So: an **open-sessions menu** listing every tab with its ADR-0107 worktree slug and live state — one button, no measurement, and unlike a pair of arrows it shows what you are navigating to; selecting a session from anywhere (menu, Sessions pane, keyboard) now **scrolls its tab into view**; and **⇧⌘[ / ⇧⌘]** step through the open tabs, wrapping, registered in `CommandRegistry` so the File menu, the palette and the ⌘/ card all pick them up. 815 Swift assertions green.

- **Switching chat tabs was quadratic, and shipped 122 MB to render 200 rows (2026-09-09, [ADR-0048 addendum](./decisions/0048-full-session-history-tail-first.md)).** User: *"switching sessions does not load the right pane smoothly and cleanly and fluid."* Measured on the real 122 MB / 36,356-turn transcript rather than reasoned about. Three causes: `selectSession` passed no `tail`, so a tab click pulled the **whole** transcript (ADR-0048 had reasoned that a history-menu pick is a deliberate act worth the cost — true until ADR-0107 made the same call the tab-strip click); `loadSession` UTF-8 decoded and `JSON.parse`d all 36,356 lines and then sliced (528 ms to produce a 200-turn answer); and `ChatStreamReducer.apply` took the message array **by value and returned a new one**, so every event copied the whole list — O(n) per event, O(n²) per replay, on the main actor. Benchmarked at the same N: **6.3 s of array copying against 6 ms in place.** That last one was invisible in the source; `rebuilt = apply(rebuilt, …)` reads as linear, and copy-on-write fires because the caller still holds what it passed. Fixes: tab switches pay the tail like every other hydrate, a new `loadSessionTail` reads the file as a Buffer and parses only the lines it returns (**113 ms**, byte-identical to load-then-slice at every tail, 7 tests), and the reducer gained an in-place `apply(to:)` that both the replay and the live stream now use. Two smaller things fell out: session switching never reset `renderWindow`, and the pane no longer blanks — rows for the last six visited tabs are kept and painted immediately, with the fetch replacing them when it lands. `hydrate` logs `turns / fetch ms / replay ms` so the next person measures. 1307 sidecar tests + 800 Swift assertions green. **Not yet verified:** the felt latency in the rebuilt app.

- **Autonomy mode, models, effort, voice and the gate are per session now (2026-09-09, [ADR-0108](./decisions/0108-per-session-posture.md)).** User: *"i believe the agent mode: Plan / Agent / Ask are not session separated… if i put agent on Plan in 1 session, the rest of the sessions will have the same mode."* Correct, and it was seven controls, not one: each was a single `UserDefaults` key and a single field on the `MarvinBridge` singleton, so a tab switched to Plan switched every open tab to Plan and the next turn any of them sent ran under it. Harmless with one chat surface; wrong the moment ADR-0107 gave every tab its own worktree. The sidecar already modelled it correctly — `SessionPosture` in the session meta records exactly these fields per session on every turn — so the fix is the client catching up: a capped MRU `SessionPostureStore` keyed by session id, `NativePrefs`' seven scalars demoted to the DEFAULTS a new tab inherits, and the bridge's posture fields redefined as "the tab on screen", which made `ChatModeToolbar` / `ChatAgentsFooter` / `TopBarPopovers` / `AppStatusBar` session-aware with no edits to any of them. `setActiveMarvinSession` applies the arriving tab's posture, so a tab switch cannot come apart from a posture switch. `GET /api/sessions/watch` now carries each row's stored posture, so a tab restored after a relaunch comes back in the mode it was left in — local record outranks the server's, which is history rather than choice. Playwright stays app-wide (a machine-level opt-in, not a per-tab one). 1300 sidecar tests + 800 Swift assertions green. **Not yet verified:** the live check on the rebuilt app.

- **One worktree per chat tab, and a way to watch every session at once (2026-09-09, [ADR-0107](./decisions/0107-one-worktree-per-tab-and-a-multi-session-watch.md)).** User: *"i have 3 sessions running simultaneously but i want marvin to be able to run multiple sessions simultaneously, perhaps we need some sort of environment constraints or sandboxes?"* The research (25 Anthropic-primary claims, 0 refuted) said: sessions persist the conversation, not the filesystem; Anthropic's own answer is one git worktree per session; sandboxes are orthogonal. Built in six slices on the implementer-worktree machinery MARVIN already had: a `workDir`/`cwd` split so no `.marvin/` service can fork into a worktree; a per-session meta beside the transcript; session worktrees with a `session` state the sweep never touches, close = Merge / Keep / Discard, `.marvin/worktree.json` + `.worktreeinclude` for deps and `.env`; main-loop containment (a write into the root is denied, shell naming it confirms) with the ADR-0102 confirm skipped in worktree mode; lanes for shared tabs; the announce stream widened into a project event bus plus `GET /api/sessions/watch`; a client `SessionRegistry` driving tab dots, a needs-you badge, the dock badge and a new **Sessions** left-pane tab; and resume-after-restart through the wakeup path with a boot-time `interrupted` marker. 1178 sidecar tests + 758 Swift assertions green. **Addendum 1, same day:** the brain was still one slot fed by the selected tab (*"some sessions might be 'thinking' some others he might be using a 'tool'"*) — the orchestrator now announces `session.activity` (thinking / writing / tool + name) on change, the ledger keeps it per session and pushes the selected tab's into the gated brain slot; and a fresh worktree arrived with no `node_modules` — MARVIN now detects git-ignored dependency dirs and env files, writes `.marvin/worktree.json` (`detected: true`) for the user to edit, and tells the model what was symlinked ([guide](./guides/worktrees.md)). **Addendum 2, same evening:** a confirm or question raised while a tab was hidden was never shown on switching to it (the attach stream carries only future events; the replay treats stored confirms as settled) — the resume route now replays still-pending confirms from the registry, and the boot-time interrupted marker honours a 7-day window instead of flagging months-old transcripts. **Addendum 3:** a turn killed by an app quit or reinstall recorded the SDK's Stop wording and showed a stream error instead of Resume — an external SIGTERM with no user cancel is now `interrupted`; and the worktree containment confirmed on *reading* a main-checkout path under the auto gate — only write-shaped mentions confirm now (refined again so a main-checkout path in a copy's SOURCE position is a read). **Addendum 4:** the stale "Stream error" on switching sessions was in the transcript REPLAY, which banners every historical error and clears none — only the last terminal event holds now, a mid-turn re-attach clears the banner, and an interrupted turn leaves it to the Resume chip. **Addendum 5:** a read-only graph query confirmed on **auto** — `cd <root>` was treated as a write, when it is the effective cwd and what runs there that decide; the containment now tracks cwd and confirms only write-shaped commands, and `graphify-out/` joins the auto-symlinked set so a tab need not leave its worktree to query the graph. **Not yet done:** the live check on the rebuilt app — the user relaunches when ready — and SDK `resume` across a mid-session tree switch is unverified (the chip only allows a switch while no turn is live).

- **Research: running and watching several sessions at once (2026-09-09).** User: *"i have 3 sessions running simultaneously but i want marvin to be able to run multiple sessions simultaneously, perhaps we need some sort of environment constraints or sandboxes?"* Deep-research run (24 sources, 25 claims verified 3-vote, 0 refuted, all Anthropic primary) in [`docs/research/2026-09-09-multi-session-management.md`](./research/2026-09-09-multi-session-management.md). Headline: the checkout collision is the documented default (sessions persist the conversation, not the filesystem); Anthropic's own answer is one worktree per session, harness-enforced since CLI 2.1.218 and the desktop app's default; sandboxes are orthogonal; agent teams are unavailable in SDK sessions. MARVIN already has the worktree machinery for implementers (ADR-0081/0103) — the recommended next step is wiring it to chat tabs with `worktree.baseRef: "head"` + `symlinkDirectories`, and a per-session strip built on `listSessions`/`getSessionInfo`/`tagSession` (all in the pinned SDK 0.3.251). No measured conflict/throughput numbers survived verification — decisions here rest on documented properties. Not started: no ADR yet.

- **The layout-loop crash is now survived, not just logged (2026-09-08, ADR-0062 addendum 5).** Fourth `NSGenericException` "more Update Constraints passes than views" in eight days, this time from a divider drag with the Practice pane open. The captured stack shows SwiftUI's two-pass sizing re-requesting a pass from inside the pass through MARVIN's own `setNeedsUpdateConstraints` swizzle — so that swizzle now enforces the limit AppKit enforces by dying: inside a pass, transitions past `max(16, views − 16)` per window are deferred to the next run-loop turn instead of forwarded. `LeftPane`'s collapse write also moved out of the geometry callback. 696 assertions green. Proof will be a `breaker tripped` entry with no crash report after it.
...
- **Two sessions in one worktree is now supported, and the collision is surfaced (2026-09-01).** User ran an MR-triage session and a production-hotfix session against one checkout and reported them "interconnected". Measured: the conversations were never joined — no SDK session id appeared in two transcripts — but the tree's reflog shows one session running `git checkout` and `git rebase` while the other edited the same files. Anthropic's documented precedent for N sessions in one directory is agent teams, which explicitly do NOT isolate and rely on partitioned ownership plus locking, so the fix surfaces rather than isolates: a HEAD-moving or history-rewriting git command now raises a confirm naming the other live session, in `auto` mode as well as `gated` (the third member of the `AskUserQuestion` family that bypasses the auto-mode bypass), and is silent for a solo session. See [ADR-0102](./decisions/0102-multiple-sessions-one-worktree.md).

- **The status bar showed the wrong session's numbers (2026-09-01).** Same report, second cause. `hydrate` reset the plan / to-do / changed-files strips per session, but the `ctx`, `graph N · reads M` and `agents` chips live on the `MarvinBridge` singleton and cleared only on a *fresh SDK session* — so switching between two live sessions left the leaving session's figures above the arriving session's transcript. The three writers sat ungated directly below two that were gated (`setMarvinState` / `setBusy`, fixed earlier the same day for the brain). All three now take `forSession:` and go through the same `BrainStateGate`, and `resetSessionCounters()` is one call shared by both reset paths.

- **A crashed turn lost its context on resume (2026-09-01).** `lastSdkSessionId` read only `turn.completed`, which a turn killed mid-flight never writes — so after the crash above, "continue." opened a brand-new SDK session instead of resuming. Visible as two SDK ids in one transcript. It now also accepts the `session_id` the SDK stamps on every `cli.event`, which is exactly what `claude --resume` does after a crash.

- **A task launched from the sidebar could not be interrupted (2026-09-01).** User: *"i started a task but i can't seem to cancel it, it run until it finished, pressing ctrl+c or STOP did nothing."* The plumbing was fine — a Tasks row types its command into the same PTY as anything else, and `TerminalSession.isRunning` tracks the **shell** rather than the command, so the terminal header's Stop (which sends ⌃C) is always present. **The defect was focus.** `PTYTerminalView` makes itself first responder in `makeNSView` only, and a pane that is already mounted never runs that again — so a task launched from the left pane left first responder in the left pane, and a reflexive ⌃C went nowhere. The Stop the user then reached for was the chat footer's, which cancels a **turn** and has nothing to do with a shell command; the one that would have worked is in the Terminal pane's own header. Fixed with a focus token bumped whenever something injects a command from elsewhere, so the shell running the job also holds the keyboard. **Not claimed as verified:** this is a focus behaviour, and focus is exactly what the headless checks cannot see — it needs a real ⌃C after a real task launch.
...
- **The constraint storm — the nested split view found and removed (2026-09-01).** `LeftPane`'s width-measurement latch was one oscillator, fixed 2026-08-31 with `onGeometryChange` + a deadband; CPU went from 100 % / `STAT R` to 2.8–6 % / `STAT S`, but **five storms still fired per launch, the same count as before**. The next occurrence was fatal — `MARVIN-2026-09-01-164458.ips`, 78 minutes in — and it was the first time the ADR-0062 pass counter captured a stack that named MARVIN's own layout: `SystemSplitView.updateNSViewController → formCurrentItems → updateRootViewForItem → NSHostingView.setRootView → setNeedsUpdate`, with **a split view nested inside a split view** in the ancestry every time. `ContentView` had a `VSplitView` (brain over chat) inside the main `HSplitView`; the outer pass re-rooted the inner's hosting view, re-dirtying the window, and AppKit's breaker raised at 78 views. Fixed by replacing the inner split with `MarvinDivider()` + a drag gesture — the same move `LeftPane` made, one level up. **Watch the count:** if five storms per launch survive this, a third path remains and the capture format will name it; if it drops, the nesting was the source. See [ADR-0062 addendum 3](./decisions/0062-update-constraints-loop-identified-mitigated.md).
...
all three are fixed in the runtime rather than proposed as rules: a bare project-local skill
name is rewritten to its plugin-namespaced form at the gate (29 "Unknown skill" failures,
every success namespaced), an identical re-run of a failed command gets an advisory nudge,
and a three-edit turn under a plan whose checklist never moved is blocked once. A project
with fewer than three sessions now shows a calibration counter and offers rules confirmed in
the user's other projects for adoption. The [Practice guide](./guides/practice.md) documents
the loop end to end. v0.1.103 in between made the four built-in gates rows the pane controls,
fit the score weights from outcomes, and let a read-only model draft a rule's wording
([ADR-0105 § Phases](./decisions/0105-practice-loop.md#phases)). Cask bumped from 0.1.101
straight to 0.1.104; 0.1.102 and 0.1.103 were released on GitHub but never reached the tap.

_The v0.1.102 summary follows._

**v0.1.102** — the practice loop, the ship-review gate, and a night of measuring MARVIN against itself.

MARVIN now reads its own sessions. A nightly, read-only pass over a project's transcripts
mines repeat failures and the same acts done right with deterministic extractors, scores
them, proposes a rule at three sessions, and enforces accepted rules at `prompt` / `nudge` /
`deny` from a data table — then measures whether they held ([ADR-0105](./decisions/0105-practice-loop.md)).
The first backtest on a real project (397 sessions, 11 s) produced six proposals, and three
of them were fixed in the runtime the same night: graph pre-orientation, a `Stop` hook for
turns that end without their handoff or with plan steps open, and the ship-review gate
([ADR-0104](./decisions/0104-ship-review-gate.md)). Also: the scrollbar change map, the chat
row that re-laid-out on every streamed event, and the half-emoji that made a session
unloadable.

_The v0.1.101 summary follows._

**v0.1.101** — IDE parity, four bugs that only measurement found, a learn-loop that proposes rather than writes, and a plan that stops forgetting it was finished.
...
- **DoD bullets are criteria, not steps — and `[x]` reads back (2026-09-07).** User showed a finished session whose plan card read **0/13** on a plan of 8 steps, with the Definition of Done listed as unchecked work items. Measured against the stored spine and the transcript before touching code: the spine held **13 steps, 12 done** — the Golden Rule 8 template puts the DoD as column-0 `- [ ]` bullets above the numbered steps, `topLevelStepRE` promoted all 5 to steps, TodoWrite `[N]` tags never aligned with the 13-step spine, the ADR-0052 rebase guard (correctly) distrusted every batch, and 47 restatement sub-tasks piled up under "Ship". The 0/13 itself was the second defect: `stepText` kept the `[x]` prefix, so a checked step's id normalized to `"x …"` and could never match its own recovery id from `completedStepIds` — every plan re-seeded from its rendered file showed 0/N (ADR-0102's bug, resurfaced one layer down). Two fixes in `PlanParser`, same shape as the ADR-0068 addendum 4 `Sources:` cut: a criteria block (`Definition of Done` / `Scope of Done` / `Acceptance criteria`) is excluded from step parsing and passed through `render` verbatim, and a leading checkbox is stripped from parsed content with its state seeding the step's status. `redriveSteps` heals an already-absorbed spine on the next reconcile (13 → 8, statuses kept). The sub-task restatement pileup (`sameWork` misses reworded progress summaries) is noticed, not in scope.
...
- **Practice cold start, and the guide (2026-09-03, v0.1.104).** A project with fewer than three sessions had nothing to propose and said so with an empty list. The Findings tab now shows a calibration card with a sessions counter, and lists rules **confirmed in the user's other projects** that this one has no rule for — *Adopt* copies one here, scoped to this project with its own verification clock; proven there, verified here, never automatic. `docs/guides/practice.md` documents the loop in one diagram, every finding kind with its paired success, the three tiers, day-two semantics and the API, linked from the README and a Guide button in the pane.

- **Three MARVIN-owned fixes from the first practice report (2026-09-03).** User: *"check if any of those need marvin bug fixing… only if it's marvin specific."* Of eight proposals, three were MARVIN's. (1) `skill.bypassed:hetzner-ssh`, 29 sessions: the skill was loaded all along, under the plugin namespace — every one of 14 successes called `marvin-project-local:hetzner-ssh`, every one of 29 failures called the bare name and got "Unknown skill", then read the folder by hand. The prompt said to use the namespaced form; prose. The gate now rewrites a bare project-local skill name to its namespaced form (`updatedInput`, the ADR-0058 mechanism). (2) `command.retried`, 26 sessions: a `PostToolUseFailure` hook remembers the failed command for the turn and an identical re-run gets an advisory nudge. (3) `plan.stale`, 48 sessions: the turn-close hook blocks, once, a ≥3-edit turn under an open plan that never touched TodoWrite. The other five are project or model behaviour and stay as proposals for the user. All three findings marked fixed, verification clocks running.

- **The practice loop, phases 2–6: coverage, the gates as rows, promotion, learned weights, drafted messages (2026-09-03).** User: *"give me the phases… what is the finish"*, then *"record these in the ADR and proceed with the implementation for all of them."* Phase 2 adds five kinds the first backtest could not see — a skill read by hand instead of invoked, a review whose findings went unacted, a plan left behind by real work, a failing command re-run verbatim, an over-budget turn — each with its paired success so it carries a rate. Phase 3 makes the four hand-written gates (graphify-first, graph-drift, advisor-on-ADR, ship-review) rows the Practice pane controls — tier, off, message, fired counts — with their logic untouched and native behaviour whenever no row exists, so the 93 gate tests did not move. Phase 6 offers to promote a project rule confirmed in two or more projects. Phase 5 fits the five score weights from every ledger's own outcomes (Spearman rank correlation, coordinate descent; cost-share ranking below eight labels) and applies them only on a click, with provenance. Phase 4 lets the user ask a read-only two-turn model session to draft a rule's message from aggregates — never a transcript, never from the runner. Order was 2 → 3 → 6 → 5 → 4 so the one SDK-touching phase landed last. Design reviewed by a Plan agent; its four refinements (seed built-ins from the pane path only, mtime-cached rows, sequence numbers on tool calls, that order) are in. See [ADR-0105 § Phases](./decisions/0105-practice-loop.md#phases).

- **The practice loop: MARVIN reads its own sessions nightly and proposes rules about its behaviour (2026-09-03).** User, from a reel about a `/dream` skill: *"each session thinks it is the first time"*, then *"wouldn't it be good to also focus on things that were good?"* Every behavioural fix in this repo came from a human reading many transcripts by hand — ADR-0067, the 2026-05-22 skill audit, ADR-0104 last night. Now a scheduled, read-only pass over `~/.marvin/sessions/<project>/` runs seven deterministic **failure extractors** (unreviewed boundary commits, graph-first skipped, stalled turns answered with a bare "continue", missing scope-met, cache re-creation, repeated gate denies, repeated errors) and four **paired success extractors**, so every finding carries a *rate* and not just a count. Findings live in a per-project ledger with exact day-two semantics (new / recurring / proposed / regressed / confirmed / dismissed-then-resurfaced, versioned extractors), are scored by a small linear model with weights the user can tune from a backtest, and cross into `proposed` at three distinct sessions. Approving one creates a **rule** enforced at `prompt`, `nudge` or `deny` from a data table in the design hooks — with ADR-0104's brakes, and a deny only where a machine-checkable discharge exists — then measured: a recurrence after acceptance is `regressed` and offered for escalation, a quiet window is `confirmed`. A **Practice** pane manages all of it: Findings, Working, Rules, Runs, run-now, backtest, schedule. No model in the loop; nothing changes without a click. Design reviewed by a read-only advisor (go-with-caveats, eight edits, all applied). See [ADR-0105](./decisions/0105-practice-loop.md).

- **A session that would not hydrate: half an emoji in the transcript (2026-09-03).** User: *"it's corrupted and this means we are stuck, we don't know where marvin was."* The app's error named it exactly: `Invalid unicode scalar value '0xde4f' … column 233434`. One `turn.user` line carried `\ude4f` — the low half of U+1FA4F — inside a background job's `…[4059 bytes elided]…` excerpt: the head/tail window sliced a JavaScript string by UTF-16 unit and kept one half of a surrogate pair. JavaScript tolerates a lone surrogate; Foundation's `JSONDecoder` rejects the whole 11.9 MB document over it. Three fixes: the window's two cuts now drop an orphaned half (`surrogateSafeHead` / `surrogateSafeTail`); `appendSessionTurn` scrubs escaped lone surrogates to U+FFFD on write, so no transcript can carry one again; `loadSession` scrubs on read, so an existing one still loads. The user's transcript was repaired in place (backup `.bak-lone-surrogate`), and the hydrate endpoint verified: 200, 5367 turns, zero lone surrogates. Nothing was lost — the last plan snapshot and the last assistant message were recoverable from the same file.

- **The chat wedged at 100 % CPU during a long thinking turn — with the 200-row render window already installed (2026-09-03).** User: *"marvin crashed again."* Not a crash: `STAT R`, CPU time growing a minute per minute, a live turn streaming ~0.5 events/s. `sample` put **1178 of 1306 main-thread samples inside `StackLayout`** across the rendered rows, 39 in platform-view measurement, none in MARVIN code — the rows were not expensive to measure, they were ALL being re-laid-out on every event. Cause: `ChatMessage` is `Equatable` but `ChatMessageRow` was not, so SwiftUI fell back to its field-wise comparison, which gives up on `ChatBlock` (an enum with payloads) and treats every row as changed. `ChatMessageRow: Equatable` + `.equatable()` in the list makes a streamed event cost one row instead of the window. The user's 18.5-minute turn was allowed to finish before the restart; the only casualty was a self-scheduled check-back wakeup. **Verify on the next long turn** by sampling: the `StackLayout` share should collapse.

- **The review skills are enforced at `git commit`, and a session audit found three more things (2026-09-02).** User: *"audit it completely and compare it to how marvin is supposed to work."* Session `8927baf0` (agri-saas-platform, 25 turns, ~$108): every mechanical rule held — sanctioned subagents only, graph-extractor writes confined to `graphify-out/`, the force-push to `main` refused twice, eight wakeup polls re-entering the session — and the prompt-only ones did not: `.gitlab-ci.yml`, `ci-fetch-secrets.sh`, a prod sudoers grant and SMTP credentials shipped across **eight pushes with `pr-review` and `security-audit` invoked 0×**, the same ~0× the 2026-05-22 audit measured for soft-nudge language. New design hook `checkShipReview` reads the diff a commit seals (index + what the same command stages) and refuses it until the personality's MUST is met: boundary paths need both skills, >3 files / >50 lines needs `pr-review`, docs-only and lockfiles pass; a review this turn covers the turn, an earlier one holds until the next commit; two denies per skill per turn then allow-and-log; fails open on git errors. Also fixed from the same audit: the wakeup path built a **different system prompt** from the chat route (no active-skills block), so every human↔wakeup transition re-created ~650K tokens of cache — 7 of 12 full re-creations, one turn **$2.67 for 100 output tokens**; one builder now. A drained queue turn recorded no `turn.started`, which made it look like a vanished turn; it does now. And the ADR-0067 breakdown script counted "waiting on you to push the tag" as a stall — a "BLOCKED on a named human action" class takes that session from **20 stalls / 2.8 h to 4 / 0.3 h**. Remaining cache misses are the Claude Code preset's per-process git-status snapshot, outside MARVIN. See [ADR-0104](./decisions/0104-ship-review-gate.md).

### 2026-08-31 → 09-01 — v0.1.94 … v0.1.101: IDE parity, and four bugs found by measuring

_Shipped. Kept in full because each carries the diagnostic trail; the short version is in the
[changelog](./history/CHANGELOG.md)._

- **A plan resurfacing in a new session came back at 0 %, and "Continue" would have redone merged work (2026-09-01).** User: *"why did marvin recreate the plan from this original session? … this can induce users in error."* A GDPR-erasure plan whose work had **shipped at 11:42** (ADR-0371, commits `a17fd132`/`019230ee`, MR !179) appeared in another session as a fresh plan file created **13:42**, showing *Paused — 1/7 steps done* with a **Continue** button — and Continue resumes from the last finished step, so pressing it invites the agent to redo merged work and the user to authorise it. **Two distinct causes, and only one is fixable mechanically.** (1) `ingestPlan` dedups by slug within the **current session's** array, while plan state is keyed `(projectId, sessionId)` and plan FILES are per project — so a plan re-presented in a different session seeded every step unticked, with the finished file sitting right there on disk. Fixed: adoption now reads `.marvin/plans/<slug>.md` and carries forward the ticked steps, keyed by `PlanProgress.normalize` of the step text exactly as `PlanStep.id` is derived — **not** by step number, because a plan revised between sessions renumbers while its steps keep their identity, and carrying progress onto whatever step now sits at that position is worse than carrying none. Only `completed` is recovered: a file records done-or-not, and inventing an `in_progress` from a `[ ]` would guess in the more dangerous direction. (2) The model re-presented a plan for work it had already shipped. **No mechanical check would have caught this instance** — no plan of that slug existed anywhere at 13:42, so from the app's view a genuinely new plan was presented, and a reliable "this plan is already done" detector is not buildable. That half is a judgement failure, and repetition is one of the session auditor's named finding classes ([ADR-0059](decisions/0059-session-auditor-runtime-dispatched-read-only.md)). 8 assertions.
