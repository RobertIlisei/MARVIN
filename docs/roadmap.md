...
- **The file tree's header was pushed off the top edge — a minHeight with no ideal (2026-08-31).** User: *"now on top the file explorer, the items are going out of bounds."* `FileTreeView().frame(minHeight: 120)` sets a minimum and **no ideal**, so the ideal-size query passes straight through to the child — and this child's ideal height is a `ScrollView`'s, which is its entire content, every file in the project. `VSplitView` asks for ideal sizes, so the tree was laid out taller than the pane; a `VStack` centres, so the overflow split top and bottom and pushed the tree's own header off the top. `paneSlot`'s existing `.clipped()` is why it vanished rather than painting over the title bar. Fixed with an explicit `idealHeight: 400` so `_FlexFrameLayout` answers the query without consulting the child (definite proposals are unaffected, so the rendered size is unchanged), plus `.clipped()` on the tree itself. **This is the same mechanism as the chat-transcript hang of 2026-08-29** — `.frame(minHeight: 140, maxHeight: .infinity)` forwarding an ideal-size query into a ScrollView — and it was reintroduced two days later by the frame added when the Outline/Timeline/Tasks split view landed. The pattern to watch: a `minHeight`-only frame over anything that scrolls.
