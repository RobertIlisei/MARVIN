/**
 * POST /api/git/unstage — body `{ cwd, paths: string[] }`
 *
 * `git restore --staged -- <paths>`. Auto-class — working tree is
 * untouched; changes stay visible in the "Changes" bucket.
 *
...
export async function POST(req: NextRequest) {
  ...
  const gate = confirmGate(req, { kind: "unstage", paths }, root);
  ...
  return NextResponse.json({ ok: true, unstaged: paths.length });
}
