// PlanModel — the plan spine's model layer (ADR-0046/0049/0052), moved out
// of TodoListView.swift so MARVINTests can exercise the reconcile logic the
// same way the app runs it (the 2026-07-02 audit found the roadmap-claimed
// reconcile tests could not exist while these types lived in the app
// target). Pure Foundation value types — no SwiftUI; the views stay in
// macos/MARVIN/TodoListView.swift.
...
public struct TodoItem: Equatable, Codable {
    ...
    public init(content: String, status: String, activeForm: String?, key: String? = nil) {
...
public enum PlanTag {
    ...
    public static func strip(_ s: String) -> String { parse(s).text }
}
...
public struct PlanStep: Equatable, Identifiable, Codable {
    ...
    public var subtasks: [TodoItem]

    public init(content: String, status: String = "pending", activeForm: String? = nil, subtasks: [TodoItem] = []) {
...
public struct Plan: Equatable, Identifiable, Codable {
    public let id: String          // PlanFile.slug(title)
    public var title: String
    public var text: String        // full plan markdown (saved to the file)
    public var path: String?       // <workDir>/.marvin/plans/<slug>.md once written
    public var steps: [PlanStep]

    public init(id: String, title: String, text: String, path: String?, steps: [PlanStep]) {
        self.id = id
        self.title = title
        self.text = text
        self.path = path
    ...
    public var isComplete: Bool { !steps.isEmpty && steps.allSatisfy { $0.status == "completed" } }
    public var doneCount: Int { steps.filter { $0.status == "completed" }.count }
}
...
public enum TodoExtractor {
    ...
    public static func todos(from data: Data) -> [TodoItem]? {
...
public enum PlanParser {
    ...
    public static func steps(from plan: String) -> [PlanStep] {
...
public enum PlanProgress {
    ...
    /// Canonical implementation lives in PlanTextMatch so this and the
    /// re-base guard share it (ADR-0052).
    public static func normalize(_ s: String) -> String { PlanTextMatch.normalize(s) }
    ...
    public static func matches(_ a: String, _ b: String) -> Bool { PlanTextMatch.matches(a, b) }
    ...
    public static func mergeSubtasks(_ existing: [TodoItem], _ incoming: [TodoItem]) -> [TodoItem] {
        var out = existing
    ...
    public static func reconcile(steps: [PlanStep], with incoming: [TodoItem]) -> [PlanStep] {
        var steps = steps
        var used = Set<Int>()
...
public enum PlanFile {
    ...
    public static func render(_ plan: Plan) -> String {
