...
enum RegexHighlighter {
    ...
    /// Language id for a file. Delegates to `MARVINLogic.LanguageDetection`,
    /// which holds the pure mapping so `MARVINTests` can pin it (ADR-0022).
    static func languageId(forExtension ext: String, filename: String? = nil) -> String? {
        LanguageDetection.languageId(forExtension: ext, filename: filename)
    }
