...
- **Memory is durable facts, not a log.** `.marvin/memory.md` is a curated,
  ...
  (ADR-0042).
- **It learns from its own sessions — and proposes, never rewrites.** Once a
  night a read-only pass over a project's transcripts finds the failures that
  repeat across sessions (and the same act done right, so each carries a
  rate), proposes a rule at three sessions, and — if you approve — enforces
  it at the tool gate and measures whether it held. No model reads a
...
- 🎛️ One vocabulary across the seven left panes — Files · Search · Source Control · Sessions · Skills · Plugins · Practice share one control set: a chip that presses and hovers, one section header with its count, real empty states that say what a pane is for, and an outcome strip that distinguishes a success from a warning from a failure and keeps the failure on screen (ADR-0114)
...
- 🌙 Practice pane — Findings · Working · Rules · Runs: what MARVIN keeps getting wrong (and right) across this project's sessions, scored, with approve / dismiss / **fixed in MARVIN** per finding, run-now, backtest and a nightly schedule. The four built-in gates are rows you can re-tier or switch off; score weights fit from real outcomes on a click; a read-only model can draft a rule's wording from aggregates. A project with fewer than three sessions shows a calibration counter and offers rules **proven in your other projects** to adopt (ADR-0105 · [guide](./docs/guides/practice.md))
...
- 📊 Graph-aware chat — in-process MCP exposes `graph_summary`, `graph_search`, `graph_neighbors`, `graph_path`; MARVIN builds + refreshes the active project's code and knowledge graphs itself (AST-only, free) so they stay current (ADR-0041). `graph_path` takes a `relations` filter and prints each hop's file, so the path from an entry symbol to the persistence it reaches is a **tracer-bullet touchpoint list** for the first milestone
- 🎯 Vertical-slice milestones — Phase 5 requires milestone 1 of any cross-layer change to be the thinnest path proving one flow through every layer it touches, verified end to end; layer-by-layer plans are refused unless the work is single-layer. The practice loop measures the shape as `plan.horizontal` / `plan.vertical`, and the **layers are discovered from your project's own graph** — its directories and file names — never a vocabulary MARVIN ships
...
**v0.1.112 — the multi-session release: nothing strands, one place for each thing, the backlog is shared.** Three decisions from three days of running four tabs at once on a real project. *Nothing survives a tab unless it holds a commit* ([ADR-0111](./docs/decisions/0111-nothing-survives-a-tab-without-a-commit.md)): measured first — 17 session worktrees on one project, 16 for closed tabs, 12 already merged, 3 empty, 0 ever swept. Empty and merged trees now go silently on close and at boot; *Keep* commits work in progress; *Merge* names the branch it merges into; a `git merge-tree` dry run reports `clean` / `behind by 2` / `conflicts in a.ts` before anything moves; **Sync** asks the tab that owns a branch to merge yours in and resolve, because it knows what its changes were for; the Sessions pane gained **Ready to integrate** with Merge / Merge all / Prepare MR; branches take their name from their first commit and an integrated tab reopens on a fresh tree. *One place for each thing* ([ADR-0112](./docs/decisions/0112-one-place-for-each-thing.md)): the chrome was counted — four ways to switch a session, two New buttons, three names for the same choice, four stacked bars — and reduced. The tab strip is the only switcher (tabs compress to fit, ‹ › step, an overflow list, ordinals for twins); History holds closed sessions and ⇧⌘[ ⇧⌘]; the header reads project then `own branch · name`; a tool confirm is a card in the tray, answerable inline from the Sessions pane; posture is one pill; ↑↓⏎ walk the pane. *The backlog is the shared task list* ([ADR-0113](./docs/decisions/0113-the-backlog-is-the-shared-task-list.md)): two tabs fixed the same bug byte-for-byte because the store lives in the project root and each worktree carries a snapshot the second tab grepped. Items are **claimed** by the tab working them (`backlog_claim`; the holder appears in every listing, in the panel badge, in the Sessions row, and in the refusal a near-duplicate gets — *"already being worked in tab fix-the-thing"*); the worktree's copy is refused at the gate in every mode; resolving an item notifies the other live tabs whose branch touches its files. Also: the app crashed with four tabs open on ADR-0062's loop through AppKit's *layout-pass* counter, which the breaker had never hooked — it does now ([ADR-0062 addendum 7](./docs/decisions/0062-update-constraints-loop-identified-mitigated.md)), and left-pane notices lost the `.textSelection` that oscillated; the close dialog offered a tab's own branch as its merge target; and **Prepare MR** squashes the day's finished tab branches into one commit on an `mr/` branch, pushes it and opens one merge request ([ADR-0109](./docs/decisions/0109-batch-integration-and-metered-ci.md) amended). The first live run of all of it found three more: Merge refused a branch because two unrelated files were unsaved on main, so it now refuses only the files the merge would actually overwrite and names them; Prepare MR reported *"the request timed out"* while it was still running, so a run is a **job** the panel watches — *"Squashing 3 of 6 · marvin/tab/fix-flags · 1m 12s"*, *"Committing — running the project's commit hooks · 2m 03s"* — whose outcome survives a timeout, a project switch and a relaunch; and a run that stopped at a conflict pushed a commit titled *"integrate 6 branches"* holding one, so a generated message now counts what applied and a stopped run does not push at all. And the panes themselves got the affordance pass the user asked for after seeing them: *"nobody knows the Prepare MR, Merge All, Reclaim are buttons, they look like normal text"* — one chip vocabulary across Source Control and Sessions, with a press cue on pointer-down, hover, a real hierarchy per section, outcomes that carry their kind instead of one grey line, a conflicted merge that names the clashing files rather than the command, and the whole-pane focus ring moved onto the row it belongs to. A review of a week's real use then closed six more: a tab closed with *Merge* was dead forever, because the chat route refused a missing checkout before reaching the recut that close-with-merge always makes necessary; pressing **Stop** was recorded as a failure, so the tab sat under a red cross and replayed "aborted by user" on every switch back, and there is a third outcome now, `stopped`, deliberately not resumable; answering a confirm from the pane wrote its clear under an empty session key and never cleared the row; a slow snapshot put answered confirms back; **Reclaim** and **Merge** skipped the busy guard, so a closed tab running a Sync turn could lose its branch mid-turn; and boot recovery minted an *open* session record on every restart, eleven of which had piled up unreachable. Last, the branch chip's popover was rendering `Switch to ow…` and `Open in Ses…` — four controls of four kinds in one row with no width, rebuilt around a fixed width and one full-width action.

**And then all seven left panes** ([ADR-0114](./docs/decisions/0114-one-vocabulary-for-the-left-pane.md)). Measured before touching anything: **five** button styles, **five** section headers, **four** corner radii, **three** registers of empty state, and a typed-notice type exactly one pane used — Apple's *Familiarity* principle failing across the whole sidebar. A shared vocabulary now lives in two halves, decisions in `MARVINLogic` where they can be asserted and drawing in `MARVIN` where they cannot, admitted by one rule: three or more panes need it, and the signature carries no pane-specific parameter. Then all seven adopted it, one per commit. The pass turned up what a survey could not: **Search opened a result's file but not its line**, though the line sat two lines from the call; **Replace All** had no confirm, no preview and no undo while writing file by file; Skills and Plugins both shipped an enable control that was a plain glyph, one of which loads MCP servers; a refused plugin toggle did nothing at all; and dismissing a Practice finding was a one-way door, so **undo** was built. Then the user dragged the sidebar narrow and found three more: rows that wrapped character by character into columns of letters, a pane that pushed the activity rail off the window, and *Generate* set as accent text on an accent fill. Sidecar 1382 tests green; 894 Swift assertions.
...
**v0.1.102–0.1.104 — the practice loop, the ship-review gate, and a night of measuring MARVIN against itself.** A session audit ([ADR-0104](./docs/decisions/0104-ship-review-gate.md)) found every mechanical rule held and every prompt-only one did not: `.gitlab-ci.yml`, a script that fetches secrets, a production sudoers grant and SMTP credentials shipped across **eight pushes with `pr-review` and `security-audit` invoked 0×** — the same ~0× the 2026-05-22 audit measured for soft-nudge language. The review skills are now enforced at `git commit` by a gate that reads the diff. Then the general form of that audit became a feature ([ADR-0105](./docs/decisions/0105-practice-loop.md)): a nightly, read-only pass over a project's transcripts runs deterministic extractors for repeat failures (unreviewed boundary commits, graph-first skipped, stalled turns, missing handoffs, skills read by hand instead of invoked, reviews whose findings went unacted, plans left behind, commands re-run verbatim) and their paired successes, scores them with weights fit from outcomes, proposes a rule at three sessions, enforces an approved one at `prompt` / `nudge` / `deny`, and measures whether it held. The first backtest on a real project — **397 sessions in 11 s** — produced six proposals; three were MARVIN's own and were fixed in the runtime the same night. v0.1.103 made the four hand-written gates rows the Practice pane controls; v0.1.104 adds the cold start (a calibration counter, and rules proven in your other projects offered for adoption), the [Practice guide](./docs/guides/practice.md), and three fixes from the first report: a bare project-local skill name rewritten to its plugin-namespaced form at the gate (29 "Unknown skill" failures, every success namespaced), an advisory nudge on an identical re-run of a failed command, and a once-per-turn block on a three-edit turn under a plan whose checklist never moved. Also: a transcript made unloadable by half an emoji (a UTF-16 slice through a surrogate pair — three fixes, nothing lost), and a chat at 100 % CPU because every row re-laid-out on every streamed event (`ChatMessageRow` was not `Equatable`).
