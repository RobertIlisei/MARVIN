...
1. **Single assistant, not an agent team.** MARVIN is one Claude session in a
   ...
   **A worktree isolates the filesystem, not the machine
   ...
   2026-09-10).** Three tabs, three checkouts, three branches — and one Docker
   daemon between them: reusable Testcontainers are keyed by config hash and
   shared across sessions, so a sibling's cleanup removed a container mid-run
   and two suites `DROP TABLE`d each other's schema. Neither the isolation nor
   the user was wrong; nothing said what isolation does *not* cover. Host
   ports, databases, global caches and anything reached through `$HOME` are
   shared by every tab. `.marvin/worktree.json` gains an `env` map applied to
   every turn in a worktree tab (and so to every subprocess it spawns) — a
   generic map, never a Testcontainers rule (Golden Rule 6), read at a
   validating boundary because a file inside the user's repo is attacker-
   controlled input. Two symptoms to recognise: tests that pass alone and fail
   when tabs overlap, and failures that move between tabs.
   **Posture is per session** ([ADR-0108](./docs/decisions/0108-per-session-posture.md))
   — permission strategy, personality, effort and the rest belong to the tab
   that set them; the app-wide values are only the defaults a new tab starts
   from. **Integration is batched** ([ADR-0109](./docs/decisions/0109-batch-integration-and-metered-ci.md)):
   `mergeAllWorktrees` folds every `ready` branch into the current branch in
   one local pass, oldest first, stopping at the first conflict and reporting
   everything it skipped with the reason — a `session` tree belongs to an open
   tab and is decided at tab close, never behind the user's back. Spending CI
   minutes asks first, in every permission mode.
...
7. **Graphify FIRST — always, before any structural file read.** For any
   ...
   `graph_path`). From a coding-agent session (Claude Code, Codex, …): the
   graphify CLI — `graphify query "…"`, `graphify path "A" "B"`,
   `graphify explain "Node"`. Files are read
...
8. **Define done before doing — match-not-improve.** For any
   ...
   `## Scope of Done` block per the template in the `marvin:adr` skill.
...
MARVIN's prompt (`sidecar/packages/runtime/src/personality.ts`) carries the
rules every turn needs; since [ADR-0118](./docs/decisions/0118-one-prompt-per-session-and-settings-isolation.md)
Milestone 3 (2026-09-21) it is ~4K tokens, down from 29.4K. The enumerated
MUST / MUST-NOT trigger lists it used to hold were retired: current Claude
models over-fire on emphatic triggers and follow a short instruction as well
as a list, and the rules that matter most are enforced by gates, not prose.
Occasional procedures load on demand as MARVIN's own skills (`core-skills.ts`,
invoked `marvin:<name>`), and memory / backlog content rules live in the tool
descriptions that enforce them. `personality-surfaces.test.ts` guards the
parsed formats, the moved rules' new homes, and an 8K-token budget. When the
prompt and a human doc disagree, the prompt wins — it's what MARVIN actually
reads at turn time.

| Rule | Location | Purpose |
|---|---|---|
| **Graphify first** | Ground-truth rule 1 in `personality.ts`; Golden Rule 7 above; the graphify-first design hooks (enforcement); the `marvin:graph-tools` skill (per-tool reference) | When to consult the graph before reading source files. The 2026-05-27 audit found ~7:1 file-ops to graph-ops drift and that `graph_search` was overused as a glorified grep while `graph_summary` / `graph_query` / `graph_save_result` were near-zero. The per-tool triggers that fixed it now live in `marvin:graph-tools`; the design hooks deny file searches until a graph call has happened; AppStatusBar surfaces the live ratio. |
| **Simplicity + surgical edits** | "Writing code" in `personality.ts` | What MARVIN MUST NOT ship (features beyond the ask, single-call-site abstractions, unrequested configurability, handling for impossible states) and how it MUST edit (no drive-by improvements to adjacent code, match the file's existing style, delete only the orphans its own change created). Adapted from the Karpathy coding guidelines (`multica-ai/andrej-karpathy-skills`, MIT); its other two principles were rejected — "ask when unclear" contradicts ADR-0067's measured anti-stall rules, and "goal-driven execution" is already Phase 5a's Definition of Done. Before this, the only coverage of overcomplication was one soft line ("Don't over-engineer") — the shape the 2026-05-22 audit found fires ~0×. |
| **Advisor triggers** | Ground-truth rule 2 + "Subagents" in `personality.ts` | When to run a Task-based advisor consult (user-directed + 7 deterministic triggers + anti-triggers). See [ADR-0007](./docs/decisions/0007-advisor-as-subagent-pattern.md) for why it's a Task subagent, not an SDK tool. A consult MUST run in the **foreground** (`run_in_background: false`) — a background second opinion cannot gate the work it exists to gate, and the gate enforces it. **A `go-with-caveats` caveat is a condition on the open scope, not deferred work** ([ADR-0100](./docs/decisions/0100-advisor-caveats-are-conditions-not-backlog.md)): conditions attach to the turn, are stated as met/waived at the scope-met handoff, and become backlog items only if the scope closes without them being met. |
| **Scout triggers** | "Subagents" in `personality.ts` | When to dispatch a read-only research subagent via `Task { subagent_type: "scout" }` (3 deterministic triggers + MUST-NOT list). See [ADR-0014](./docs/decisions/0014-scout-subagents-read-only.md) for the SDK-level read-only enforcement. |
| **Dynamic workflows** | "Subagents" in `personality.ts` | When `effort: xhigh` may fan out parallel subagents — read-only audit / research / discovery ONLY, opt-in, never parallel implementation. Enforced by the subagent read-only invariant in `classifyToolCall` (any `agentID` call that mutates is hard-denied). See [ADR-0030](./docs/decisions/0030-dynamic-workflows-read-only-fan-out.md). |
| **ADR triggers** | The `marvin:adr` skill (triggers, anti-triggers, template); "How you work" step 4 points at it | When a decision requires an ADR (9 categories + anti-triggers + re-derivation test) |
| **Definition of Done** | "How you work" steps 5 and 7 in `personality.ts` + the ADR template's `## Scope of Done` (`marvin:adr`) | Bound scope before coding; verify against the DoD; end real-work turns with explicit handoff. See Golden Rule 8 above. |
| **Negative claims** | "Honesty" in `personality.ts`; [ADR-0068](./docs/decisions/0068-plan-dedupe-provenance-and-negative-claims.md) | What MARVIN must establish before saying something doesn't exist, was never done, or was fabricated. On 2026-08-17 it scanned 303 plan files, missed the active plan, and reported that genuine merged work was "fabricated" — the user was one step from discarding it. MUST resolve by identity (the active-plan block now names `id` + `source:` path), search ≥2 ways, and state where it looked; MUST-NOT call the user's project history fabricated without positively establishing the negative. |
| **Scope-boundary gating** | "Scope boundaries" and "Questions" in `personality.ts`; [ADR-0067](./docs/decisions/0067-gate-on-scope-not-turn-boundaries.md) | When to CONTINUE inside an approved plan vs STOP and hand off, and which questions are stalls rather than questions. The 2026-08-17 transcript analysis found 33.1 h of a 49 h session spent waiting on the user, ~90 % of it avoidable — 65 turns ended mid-plan with no question asked, 20 asked permission the plan had already granted, and the user typed a "resume the ACTIVE plan" macro 8× to restart a stalled system. Transport errors now auto-continue via the ADR-0031 wakeup rails instead of leaving the session dead (5.1 h across 4 incidents). |
| **Skill triggers** | "Skills" in `personality.ts` (one line per skill) + the ship-review gate below | When to invoke `test-driven-development`, `systematic-debugging`, `pr-review`, `security-audit`, `frontend-design` via the `Skill` tool (per-skill MUST + MUST-NOT). The 2026-05-22 audit found 5 of 6 skills had soft-nudge language and fired ~0× across thousands of qualifying contexts; the per-skill MUST blocks that fixed it were retired with ADR-0118, because the ship-review gate below now enforces the two that matter at commit. |
| **Practice rules** | `checkPracticeRules` in `design-hooks.ts` + `practicePromptBlock` in the prompt builder; `practice.ts` / `practice-extractors.ts`; [ADR-0105](./docs/decisions/0105-practice-loop.md) | Rules the user accepted from the **practice loop** — a nightly, read-only pass over a project's own transcripts that mines repeat failures (and the same acts done right) with deterministic extractors, scores them, and proposes a rule at three sessions. Accepted rules enforce at `prompt` / `nudge` / `deny` tier from a data table, with ADR-0104's brakes (a deny needs a discharge path, measure mode, two-per-turn cap, bypass logged), and are verified: a recurrence after acceptance is `regressed`, a quiet window is `confirmed`. Managed end to end from the Practice pane. Since v0.1.103 the four hand-written gates are themselves rows (`builtin:*`) — tier, on/off and message editable there, native when no row exists — and the pane can fit the score weights from outcomes and ask a read-only model to draft a rule's message from aggregates. Since v0.1.104 a project with fewer than three sessions shows a calibration counter and can *Adopt* rules confirmed in the user's other projects. Nothing is written without approval. User guide: [`docs/guides/practice.md`](./docs/guides/practice.md). |
...
| **Project-controlled config** | "Always check" in [`REVIEW.md`](./REVIEW.md); [ADR-0110](./docs/decisions/0110-worktrees-isolate-the-filesystem-not-the-machine.md) | A file inside the user's repository is written by whoever wrote the repository. A reader whose values reach a subprocess environment, an argv, an executed path or an interpreter option must validate at the read boundary, refuse the names that select code (`PATH`, `DYLD_*`/`LD_*`, `NODE_OPTIONS`, `PYTHONPATH`, `GIT_SSH_COMMAND`) and MARVIN's own `CLAUDE_*`/`MARVIN_*`/`ANTHROPIC_*` prefixes, and be applied so MARVIN-owned values still win. Caught on the `env` map's own pre-landing review, 2026-09-10 — cloning a hostile repo would otherwise have been arbitrary code execution. The same review rule covers **declared records nothing writes** (`confirm.decision` had 0 writers across every transcript of a real project) and **isolation claims that don't state their boundary** (a worktree isolates the tree and the branch, never the machine). |
| **Plan spine** | `PlanModel.swift` + `plan-spine.ts`; [ADR-0046](./docs/decisions/0046-plan-as-durable-spine.md) / [ADR-0049](./docs/decisions/0049-plan-step-join-key-and-rollup.md) / [ADR-0052](./docs/decisions/0052-durable-plan-spine-and-plan-file-ownership.md) / [ADR-0068](./docs/decisions/0068-plan-dedupe-provenance-and-negative-claims.md) / [ADR-0106](./docs/decisions/0106-plan-snapshot-semantics-superseded-subtasks.md) / [ADR-0116](./docs/decisions/0116-the-spine-has-two-writers.md) | The tracked plan: `TodoWrite` snapshots reconciled by `[N]`/`[N.M]` tag, the file under `.marvin/plans/` is the **app's** projection (a direct model write is denied at the gate). A criteria block (Definition of Done / Scope of Done / Acceptance criteria) is excluded from step parsing and passed through verbatim, and `stepText` strips a leading checkbox so a step's id is identical whether it came from the chat or the rendered file (v0.1.106 — an agri-saas plan read `0/13` on 8 steps with the work done). **What is NOT a step is now four rules, all in `PlanParser` ([ADR-0068 addendum 5](./docs/decisions/0068-plan-dedupe-provenance-and-negative-claims.md)):** a citation, a criteria bullet (the block survives the blank line under its heading, and numbered criteria count — the scanner and the step matcher share `isListItemLine`), anything outside an explicit `## Steps` / `## Implementation` section when the plan names one (`Milestone` headings are step *groups*, not the section; an empty section falls back to the whole document), and a bold ordinal IS a step (`**1. …**` — both matchers and `render`'s `markerRE` accept leading emphasis, or parse and render drift). Since [ADR-0106](./docs/decisions/0106-plan-snapshot-semantics-superseded-subtasks.md): `TodoWrite` is a full-list rewrite, so a sub-task dropped by the batch that closes its parent is marked **`superseded`** (`[-]`) — never `completed` (a tick would be a false claim), never deleted (the file keeps the evidence). Since [ADR-0116](./docs/decisions/0116-the-spine-has-two-writers.md) the spine has **two writers**: the sidecar projects every turn's `TodoWrite` into it by the deterministic `[N]` join alone, so a turn nobody is watching still advances the plan, and refuses what it cannot join rather than guessing; the fuzzy half stays in Swift. The scope-met guard reads the spine AND the batch, either saying "open" makes it open, and `lastTodoAt` orders a replayed batch against what the server has already seen. |
| **`/refine`** | [ADR-0101](./docs/decisions/0101-refine-proposes-practice-lessons.md) | Reviews the session's trajectory and **proposes** durable updates; it writes nothing. Every proposal is routed through an existing enforced write path (`remember` for a fact, `backlog_add` for work), so the ADR-0042 / ADR-0044 caps and content-class rejections still apply — `/refine` adds no new write channel. One new content class, `practice`: *"we tried X, it was wrong, do Y instead"* about how to work in this repo, and a proposal that cannot cite what happened in the session is rejected at the boundary. Consent is a keep/dismiss review (ADR-0047 shape), never model confidence. The base prompt (`personality.ts`) is immutable to it. |
| **Worktree lifecycle** | `worktrees.ts`; [ADR-0103](./docs/decisions/0103-implementer-branch-lifecycle.md) (amended by [ADR-0107](./docs/decisions/0107-one-worktree-per-tab-and-a-multi-session-watch.md)) | `reconcileWorktrees` **derives** state from git on every read — ref gone → dropped, bound task inside the grace window → `running`, `rev-list --count base..branch == 0` → `empty`, `git branch --all --contains` names another ref → `merged`, else `ready`; plus `session` while a chat tab holds it (never swept, no staleness timer). Load-bearing because **the user merges in a terminal, in another session, or by hand, and MARVIN is not there to see it** — a recorded state would be wrong the instant it did. What is provably spent is reclaimed automatically; what holds work is never touched without an explicit choice. |
| **Project memory** | "Memory, backlog and vault" in `personality.ts` + the `remember` tool description; [ADR-0042](./docs/decisions/0042-memory-as-durable-facts.md) | What may be written to `.marvin/memory.md` and how. Durable facts only (invariants / gotchas / constraints / external facts), via the `remember` MCP tool — MUST-NOT Edit/Write memory.md directly or log activity/decisions/status. The 2026-06-14 audit found a project's memory.md at 419 KB / ~99 % redundant with ADRs/git/changelog; the tool enforces brevity + content-class at the write boundary where prose guidance failed. Since 2026-07-02 the gate also hard-denies direct model writes to `.marvin/memory.md` / `.marvin/memory/` (ADR-0042 enforcement addendum — same mechanism as the `.marvin/plans/` deny). |
| **Project backlog** | "Memory, backlog and vault" in `personality.ts` + the `backlog_*` tool descriptions; [ADR-0044](./docs/decisions/0044-project-backlog.md) | What may be parked to `.marvin/backlog/` and how. Actionable deferred work only ("noticed in flight, not in scope" follow-ups / out-of-scope improvements / blockers), via the `backlog_add` MCP tool — MUST-NOT park facts (→`remember`), status (→git), or decisions (→ADR). **Anti-Kanban (Golden Rule 1):** a parking lot read by MARVIN + the user — no subagent pull, never auto-executed, never overrides plan-first. Capture is un-gated at discovery (`provisional: true`, ADR-0047); consent moves to the keep/dismiss review at the scope-met handoff; open items surface in the next session's context. |

Since ADR-0118 the pattern is: one plain rule in the prompt, the procedure in
a skill or tool description, and a gate wherever a rule must hold regardless of
what the model does.
...
`sidecar/packages/runtime/src/personality.ts` ▸ "Skills" gives each one a
one-line trigger. If you add a skill MARVIN should reach for, add its line
there.
...
([ADR-0118](./docs/decisions/0118-one-prompt-per-session-and-settings-isolation.md)).
They always match the running version — which `install-skills.sh`, which never
updates, could not guarantee. Edit them in `core-skills.ts`; a test checks that
every `marvin:` pointer resolves and every core skill is pointed at.
...
Common shapes MARVIN reaches for (documented in the `marvin:browser` skill):
...
1. Sketch the approach. For anything non-trivial, add a one-line entry under
   ...
   [`docs/decisions/`](./docs/decisions/) — the `marvin:adr` skill
   (`core-skills.ts`) lists when one is required.
2. Implement.
3. When it lands, move the roadmap entry from `## In flight` to the
   ...
   diagnostic / decision / verification trail. A release also updates the
   README's "What makes MARVIN different" / Features / Status, the white
   paper and the Technical Reference **as content, not just the version
   string** — then regenerate the PDFs with
   `node scripts/whitepaper-pdf.mjs docs/whitepaper/<NAME>.md docs/whitepaper/<NAME>.pdf`
   (they were two months stale by v0.1.104).
...
In a coding-agent session on MARVIN's own source, rebuild manually (the
